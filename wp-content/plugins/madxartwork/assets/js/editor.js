...
ControlBaseDataView = ControlBaseView.extend({
  ui: function ui() {
    ...
    _.extend(ui, {
      ...
      responsiveSwitchers: '.madxartwork-responsive-switcher',
  ...
  behaviors: function behaviors() {
    ...
    if (dynamicSettings && dynamicSettings.active) {
      var tags = _.filter(madxartwork.dynamicTags.getConfig('tags'), function (tag) {
  ...
  onResponsiveSwitchersClick: function onResponsiveSwitchersClick(event) {
    ...
    madxartwork.changeDeviceMode(device);
  },
  renderResponsiveSwitchers: function renderResponsiveSwitchers() {
    var templateHtml = Marionette.Renderer.render('#tmpl-madxartwork-control-responsive-switchers', this.model.attributes);
...
ControlBaseMultipleItemView = ControlBaseDataView.extend({
  ...
  getControlValue: function getControlValue(key) {
    ...
    return madxartworkCommon.helpers.cloneObject(values);
  },
...
ControlSelect2ItemView = ControlBaseDataView.extend({
  ...
  getSelect2DefaultOptions: function getSelect2DefaultOptions() {
    return {
      ...
      dir: madxartworkCommon.config.isRTL ? 'rtl' : 'ltr'
    };
  },
...
ControlBaseView = Marionette.CompositeView.extend({
  ui: function ui() {
    return {
      controlTitle: '.madxartwork-control-title'
    };
  },
  behaviors: function behaviors() {
    var behaviors = {};
    return madxartwork.hooks.applyFilters('controls/base/behaviors', behaviors, this);
  },
  ...
  className: function className() {
    ...
    var classes = 'madxartwork-control madxartwork-control-' + this.model.get('name') + ' madxartwork-control-type-' + this.model.get('type'),
    ...
    if (!_.isEmpty(responsive)) {
      classes += ' madxartwork-control-responsive-' + responsive.max;
    }
  ...
  getTemplate: function getTemplate() {
    return Marionette.TemplateCache.get('#tmpl-madxartwork-control-' + this.model.get('type') + '-content');
  },
  initialize: function initialize(options) {
    this.elementSettingsModel = options.elementSettingsModel;
    var controlType = this.model.get('type'),
        controlSettings = jQuery.extend(true, {}, madxartwork.config.controls[controlType], this.model.attributes);
  ...
  toggleControlVisibility: function toggleControlVisibility() {
    var isVisible = madxartwork.helpers.isActiveControl(this.model, this.elementSettingsModel.attributes);
    this.$el.toggleClass('madxartwork-hidden-control', !isVisible);
    madxartwork.getPanelView().updateScrollbar();
  },
  onRender: function onRender() {
    var layoutType = this.model.get('label_block') ? 'block' : 'inline',
        showLabel = this.model.get('show_label'),
        elClasses = 'madxartwork-label-' + layoutType;
    elClasses += ' madxartwork-control-separator-' + this.model.get('separator');

    if (!showLabel) {
      elClasses += ' madxartwork-control-hidden-label';
    }
...
BaseElementView = BaseContainer.extend({
  ...
  renderAttributes: {},
  className: function className() {
    var classes = 'madxartwork-element madxartwork-element-edit-mode ' + this.getElementUniqueID();

    if (this.toggleEditTools) {
      classes += ' madxartwork-element--toggle-edit-tools';
    }
  ...
  ui: function ui() {
    return {
      tools: '> .madxartwork-element-overlay > .madxartwork-editor-element-settings',
      editButton: '> .madxartwork-element-overlay .madxartwork-editor-element-edit',
      duplicateButton: '> .madxartwork-element-overlay .madxartwork-editor-element-duplicate',
      addButton: '> .madxartwork-element-overlay .madxartwork-editor-element-add',
      removeButton: '> .madxartwork-element-overlay .madxartwork-editor-element-remove'
    };
  },
  behaviors: function behaviors() {
    var groups = madxartwork.hooks.applyFilters('elements/' + this.options.model.get('elType') + '/contextMenuGroups', this.getContextMenuGroups(), this);
    var behaviors = {
      contextMenu: {
    ...
    return madxartwork.hooks.applyFilters('elements/base/behaviors', behaviors, this);
  },
  getBehavior: function getBehavior(name) {
    return this._behaviors[(0, _keys.default)(this.behaviors()).indexOf(name)];
  },
  ...
  getChildType: function getChildType() {
    return madxartwork.helpers.getElementChildType(this.getElementType());
  },
  getChildView: function getChildView(model) {
    ...
    if ('section' === elType) {
      ...
      ChildView = __webpack_require__(375);
    } else {
      ChildView = madxartwork.modules.elements.views.Widget;
    }

    return madxartwork.hooks.applyFilters('element/view', ChildView, model, this);
  },
  ...
  getContextMenuGroups: function getContextMenuGroups() {
    ...
    return [{
      ...
      actions: [{
        name: 'edit',
        icon: 'eicon-edit',
        title: madxartwork.translate('edit_element', [this.options.model.getTitle()]),
        callback: this.options.model.trigger.bind(this.options.model, 'request:edit')
      }, {
        name: 'duplicate',
        icon: 'eicon-clone',
        title: madxartwork.translate('duplicate'),
      ...
      actions: [{
        name: 'copy',
        title: madxartwork.translate('copy'),
        shortcut: controlSign + '+C',
        callback: this.copy.bind(this)
      }, {
        name: 'paste',
        title: madxartwork.translate('paste'),
        shortcut: controlSign + '+V',
        callback: this.paste.bind(this),
        isEnabled: this.isPasteEnabled.bind(this)
      }, {
        name: 'pasteStyle',
        title: madxartwork.translate('paste_style'),
        shortcut: controlSign + '+⇧+V',
        callback: this.pasteStyle.bind(this),
        isEnabled: function isEnabled() {
          return !!madxartworkCommon.storage.get('transfer');
        }
      }, {
        name: 'resetStyle',
        title: madxartwork.translate('reset_style'),
      ...
      actions: [{
        name: 'delete',
        icon: 'eicon-trash',
        title: madxartwork.translate('delete'),
        shortcut: '⌦',
  ...
  getHandlesOverlay: function getHandlesOverlay() {
    var $handlesOverlay = jQuery('<div>', {
      class: 'madxartwork-element-overlay'
    }),
        $overlayList = jQuery('<ul>', {
      class: "madxartwork-editor-element-settings madxartwork-editor-".concat(this.getElementType(), "-settings")
    });
    jQuery.each(this.getEditButtons(), function (toolName, tool) {
      var $item = jQuery('<li>', {
        class: "madxartwork-editor-element-setting madxartwork-editor-element-".concat(toolName),
        title: tool.title
      }),
          $icon = jQuery('<i>', {
  ...
  startTransport: function startTransport(type) {
    madxartworkCommon.storage.set('transfer', {
      type: type,
  ...
  isPasteEnabled: function isPasteEnabled() {
    var transferData = madxartworkCommon.storage.get('transfer');
  ...
  duplicate: function duplicate() {
    var oldTransport = madxartworkCommon.storage.get('transfer');
    this.copy();
    this.paste();
    madxartworkCommon.storage.set('transfer', oldTransport);
  },
  pasteStyle: function pasteStyle() {
    var self = this,
        transferData = madxartworkCommon.storage.get('transfer'),
    ...
    jQuery.each(controls, function (controlName, control) {
      ...
      var ControlView = madxartwork.getControlView(control.type);
    ...
    madxartwork.channels.data.trigger('element:before:paste:style', editModel);
    editModel.setSetting(diffSettings);
    madxartwork.channels.data.trigger('element:after:paste:style', editModel);
  ...
  resetStyle: function resetStyle() {
    var self = this,
        ...
        defaultValues = {};
    self.allowRender = false;
    madxartwork.channels.data.trigger('element:before:reset:style', editModel);
    jQuery.each(controls, function (controlName, control) {
      if (!self.isStyleTransferControl(control)) {
        return;
      }
    ...
    madxartwork.channels.data.trigger('element:after:reset:style', editModel);
    self.allowRender = true;
    self.renderOnChange();
  },
  ...
  toggleVisibilityClass: function toggleVisibilityClass() {
    this.$el.toggleClass('madxartwork-edit-hidden', !!this.model.get('hidden'));
  },
  addElementFromPanel: function addElementFromPanel(options) {
    options = options || {};
    var elementView = madxartwork.channels.panelElements.request('element:selected');
    var itemData = {
      elType: elementView.model.get('elType')
    };

    if (madxartwork.helpers.maybeDisableWidget()) {
      return;
    }
  ...
  enqueueFonts: function enqueueFonts() {
    ...
    jQuery.each(settings.getFontControls(), function (index, control) {
      ...
      madxartwork.helpers.enqueueFont(fontFamilyName);
    }); // Enqueue Icon Fonts
    ...
    jQuery.each(settings.getIconsControls(), function (index, control) {
      ...
      madxartwork.helpers.enqueueIconFonts(iconType.library);
    });
  },
  renderStyles: function renderStyles(settings) {
    ...
    this.controlsCSSParser.addStyleRules(settings.getStyleControls(), settings.attributes, this.getEditModel().get('settings').controls, [/{{ID}}/g, /{{WRAPPER}}/g], [this.getID(), '#madxartwork .' + this.getElementUniqueID()]);
    this.controlsCSSParser.addStyleToDocument();
    var extraCSS = madxartwork.hooks.applyFilters('editor/style/styleText', '', this);
  ...
  renderCustomClasses: function renderCustomClasses() {
    ...
    _.each(classControls, function (control) {
      ...
      var isVisible = madxartwork.helpers.isActiveControl(control, settings.attributes);
  ...
  runReadyTrigger: function runReadyTrigger() {
    ...
    _.defer(function () {
      madxartworkFrontend.elementsHandler.runReadyTrigger(self.el);
      ...
      self.$el.find('.madxartwork-element.madxartwork-' + self.model.get('elType') + ':not(.madxartwork-element-edit-mode)').each(function () {
        madxartworkFrontend.elementsHandler.runReadyTrigger(this);
      });
    });
  },
  ...
  getElementUniqueID: function getElementUniqueID() {
    return 'madxartwork-element-' + this.getID();
  },
  renderOnChange: function renderOnChange(settings) {
    ...
    if (settings instanceof madxartworkModules.editor.elements.models.BaseSettings) {
  ...
  removeElement: function removeElement() {
    madxartwork.channels.data.trigger('element:before:remove', this.model);
    var parent = this._parent;
    parent.isManualRemoving = true;
    this.model.destroy();
    parent.isManualRemoving = false;
    madxartwork.channels.data.trigger('element:after:remove', this.model);
  },
  ...
  onRender: function onRender() {
    ...
    if (this.toggleEditTools) {
      ...
      this.ui.tools.hoverIntent(function () {
        editButton.addClass('madxartwork-active');
      }, function () {
        editButton.removeClass('madxartwork-active');
      }, {
        timeout: 500
      });
    }
  ...
  onCollectionChanged: function onCollectionChanged() {
    madxartwork.saver.setFlagEditorChange(true);
  },
  onEditSettingsChanged: function onEditSettingsChanged(changedModel) {
    madxartwork.channels.editor.trigger('change:editSettings', changedModel, this);
  },
  ...
  onEditRequest: function onEditRequest() {
    ...
    if ('edit' !== madxartwork.channels.dataEditMode.request('activeMode')) {
      return;
    }

    var model = this.getEditModel(),
        panel = madxartwork.getPanelView();
    ...
    if (options.scrollIntoView) {
      madxartwork.helpers.scrollToView(this.$el, 200);
    }
  ...
  onMouseDown: function onMouseDown(event) {
    if (jQuery(event.target).closest('.madxartwork-inline-editing').length) {
      return;
    }

    madxartworkFrontend.elements.window.document.activeElement.blur();
  },
  onDestroy: function onDestroy() {
    this.controlsCSSParser.removeStyleFromDocument();
    this.getEditModel().get('settings').validators = {};
    madxartwork.channels.data.trigger('element:destroy', this.model);
  }
});
...
var ContextMenu = __webpack_require__(373);
...
module.exports = Marionette.Behavior.extend({
  ...
  initContextMenu: function initContextMenu() {
    ...
    contextMenuGroups.splice(afterGroupIndex, 0, {
      ...
      actions: [{
        name: 'navigator',
        title: madxartwork.translate('navigator'),
  ...
  onContextMenu: function onContextMenu(event) {
    if ($e.shortcuts.isControlEvent(event) || !madxartwork.userCan('design')) {
      return;
    }
    ...
    madxartwork.channels.editor.reply('contextMenu:targetView', this.view);
  },
  ...
  onContextMenuHide: function onContextMenuHide() {
    madxartwork.channels.editor.reply('contextMenu:targetView', null);
  },
  onDestroy: function onDestroy() {
    if (this.contextMenu) {
...
function (_Marionette$LayoutVie) {
  ...
  (0, _createClass2.default)(_default, [{
    ...
    value: function initModal() {
      var modalOptions = {
        className: 'madxartwork-templates-modal',
        closeButton: false,
        draggable: false,
        hide: {
      ...
      this.modal = madxartworkCommon.dialogsManager.createWidget('lightbox', modalOptions);
...
function (_Marionette$LayoutVie) {
  ...
  (0, _createClass2.default)(_default, [{
    ...
    value: function className() {
      return 'madxartwork-templates-modal__header';
    }
  }, {
    key: "getTemplate",
    value: function getTemplate() {
      return '#tmpl-madxartwork-templates-modal__header';
    }
  }, {
    key: "regions",
    value: function regions() {
      return {
        logoArea: '.madxartwork-templates-modal__header__logo-area',
        tools: '#madxartwork-template-library-header-tools',
        menuArea: '.madxartwork-templates-modal__header__menu-area'
      };
    }
  }, {
    key: "ui",
    value: function ui() {
      return {
        closeModal: '.madxartwork-templates-modal__header__close'
      };
    }
  }, {
    key: "events",
...
function (_Marionette$ItemView) {
  ...
  (0, _createClass2.default)(_default, [{
    key: "getTemplate",
    value: function getTemplate() {
      return '#tmpl-madxartwork-templates-modal__header__logo';
    }
  }, {
    key: "className",
    value: function className() {
      return 'madxartwork-templates-modal__header__logo';
    }
  }, {
    key: "events",
    value: function events() {
...
function (_Marionette$ItemView) {
  ...
  (0, _createClass2.default)(_default, [{
    key: "id",
    value: function id() {
      return 'madxartwork-template-library-loading';
    }
  }, {
    key: "getTemplate",
    value: function getTemplate() {
      return '#tmpl-madxartwork-template-library-loading';
    }
  }]);
  return _default;
}(Marionette.ItemView);
...
ControlsCSSParser = madxartworkModules.ViewModule.extend({
  stylesheet: null,
  getDefaultSettings: function getDefaultSettings() {
    return {
      id: 0,
  ...
  getDefaultElements: function getDefaultElements() {
    return {
      $stylesheetElement: jQuery('<style>', {
        id: 'madxartwork-style-' + this.getSettings('id')
      })
    };
  },
  initStylesheet: function initStylesheet() {
    var breakpoints = madxartworkFrontend.config.breakpoints;
  ...
  addRepeaterControlsStyleRules: function addRepeaterControlsStyleRules(repeaterValues, repeaterControlsItems, controls, placeholders, replacements) {
    ...
    repeaterControlsItems.forEach(function (item, index) {
      ...
      self.addStyleRules(item, itemModel.attributes, controls, placeholders.concat(['{{CURRENT_ITEM}}']), replacements.concat(['.madxartwork-repeater-item-' + itemModel.get('_id')]));
    });
  },
  addDynamicControlStyleRules: function addDynamicControlStyleRules(value, control) {
    var self = this;
    madxartwork.dynamicTags.parseTagsText(value, control.dynamic, function (id, name, settings) {
      var tag = madxartwork.dynamicTags.createTag(id, name, settings);
      ...
      self.addStyleRules(tagSettingsModel.getStyleControls(), tagSettingsModel.attributes, tagSettingsModel.controls, ['{{WRAPPER}}'], ['#madxartwork-tag-' + id]);
    });
  },
  addStyleToDocument: function addStyleToDocument() {
    madxartwork.$previewContents.find('head').append(this.elements.$stylesheetElement);
...
ControlsCSSParser.addControlStyleRules = function (stylesheet, control, controls, valueCallback, placeholders, replacements) {
  ...
  _.each(control.selectors, function (cssProperty, selector) {
    ...
    if (!(0, _keys.default)(query).length && control.responsive) {
      query = _.pick(madxartworkCommon.helpers.cloneObject(control.responsive), ['min', 'max']);
...
ControlsCSSParser.parsePropertyPlaceholder = function (control, value, controls, valueCallback, placeholder, parserControlName) {
  ...
  return madxartwork.getControlView(control.type).getStyleValue(placeholder, value, control);
};
...
SortableBehavior = Marionette.Behavior.extend({
  ...
  initialize: function initialize() {
    this.listenTo(madxartwork.channels.dataEditMode, 'switch', this.onEditModeSwitched).listenTo(this.view.options.model, 'request:sort:start', this.startSort).listenTo(this.view.options.model, 'request:sort:update', this.updateSort).listenTo(this.view.options.model, 'request:sort:receive', this.receiveSort);
  },
  ...
  onRender: function onRender() {
    ...
    _.defer(function () {
      self.onEditModeSwitched(madxartwork.channels.dataEditMode.request('activeMode'));
    });
  },
  onDestroy: function onDestroy() {
    this.deactivate();
  ...
  activate: function activate() {
    ...
    var $childViewContainer = this.getChildViewContainer(),
      ...
      placeholder: 'madxartwork-sortable-placeholder madxartwork-' + this.getOption('elChildType') + '-placeholder',
      cursorAt: {
        top: 20,
        left: 25
      },
      helper: this._getSortableHelper.bind(this),
      cancel: 'input, textarea, button, select, option, .madxartwork-inline-editing, .madxartwork-tab-title'
    },
  ...
  _getSortableHelper: function _getSortableHelper(event, $item) {
    ...
    return '<div style="height: 84px; width: 125px;" class="madxartwork-sortable-helper madxartwork-sortable-helper-' + model.get('elType') + '"><div class="icon"><i class="' + model.getIcon() + '"></i></div><div class="madxartwork-element-title-wrapper"><div class="title">' + model.getTitle() + '</div></div></div>';
  },
  ...
  startSort: function startSort(event, ui) {
    ...
    madxartwork.channels.data.reply('dragging:model', model).reply('dragging:parent:view', this.view).trigger('drag:start', model).trigger(model.get('elType') + ':drag:start');
  },
  updateSort: function updateSort(ui) {
    var model = madxartwork.channels.data.request('dragging:model'),
  ...
  receiveSort: function receiveSort(event, ui) {
    ...
    this.view.addChildElement(modelData, {
      ...
      onAfterAdd: function onAfterAdd() {
        var senderSection = madxartwork.channels.data.request('dragging:parent:view');
  ...
  onSortOver: function onSortOver(event) {
    event.stopPropagation();
    var model = madxartwork.channels.data.request('dragging:model');
    jQuery(event.target).addClass('madxartwork-draggable-over').attr({
      'data-dragged-element': model.get('elType'),
      'data-dragged-is-inner': model.get('isInner')
    });
    this.$el.addClass('madxartwork-dragging-on-child');
  },
  onSortOut: function onSortOut(event) {
    event.stopPropagation();
    jQuery(event.target).removeClass('madxartwork-draggable-over').removeAttr('data-dragged-element data-dragged-is-inner');
    this.$el.removeClass('madxartwork-dragging-on-child');
  },
...
InnerTabsBehavior = Marionette.Behavior.extend({
  ...
  handleInnerTabs: function handleInnerTabs(parent) {
    var closedClass = 'madxartwork-tab-close',
        activeClass = 'madxartwork-tab-active',
        tabsWrappers = parent.children.filter(function (view) {
      return 'tabs' === view.model.get('type');
    });

    _.each(tabsWrappers, function (view) {
      view.$el.find('.madxartwork-control-content').remove();
...
module.exports = madxartworkModules.ViewModule.extend({
  ...
  bindEvents: function bindEvents() {
    madxartwork.on('preview:loaded', this.onmadxartworkPreviewLoaded);
    this.model.on('change', this.onModelChange);
  },
  addPanelPage: function addPanelPage() {
    var name = this.getSettings('name');
    madxartwork.getPanelView().addPage(name + '_settings', {
      view: madxartwork.settings.panelPages[name] || madxartwork.settings.panelPages.base,
  ...
  initModel: function initModel() {
    this.model = new madxartworkModules.editor.elements.models.BaseSettings(this.getSettings('settings'), {
  ...
  save: function save(callback) {
    ...
    NProgress.start();
    madxartworkCommon.ajax.addRequest('save_' + this.getSettings('name') + '_settings', {
      data: data,
  ...
  addPanelMenuItem: function addPanelMenuItem() {
    ...
    madxartwork.modules.layouts.panel.pages.menu.Menu.addItem(menuItemOptions, 'settings', menuSettings.beforeItem);
  },
...
module.exports = Marionette.Region.extend({
  ...
  constructor: function constructor() {
    ...
    var savedStorage = madxartworkCommon.storage.get(this.getStorageKey());
  ...
  saveStorage: function saveStorage(key, value) {
    ...
    madxartworkCommon.storage.set(this.getStorageKey(), this.storage);
  },
  saveSize: function saveSize() {
    this.saveStorage('size', madxartwork.helpers.getElementInlineStyle(this.$el, this.storageSizeKeys));
  }
});
...
var isObject = __webpack_require__(21);
var setPrototypeOf = __webpack_require__(332).set;
...
InsertTemplateHandler = Marionette.Behavior.extend({
  ui: {
    insertButton: '.madxartwork-template-library-template-insert'
  },
  events: {
    'click @ui.insertButton': 'onInsertButtonClick'
  },
  onInsertButtonClick: function onInsertButtonClick() {
    var autoImportSettings = madxartwork.config.document.remoteLibrary.autoImportSettings;
    ...
    madxartwork.templates.importTemplate(this.view.model, {
  ...
  showImportDialog: function showImportDialog(model) {
    ...
    dialog.onConfirm = function () {
      madxartwork.templates.importTemplate(model, {
  ...
  initDialog: function initDialog() {
    InsertTemplateHandler.dialog = madxartworkCommon.dialogsManager.createWidget('confirm', {
      id: 'madxartwork-insert-template-settings-dialog',
      headerMessage: madxartwork.translate('import_template_dialog_header'),
      message: madxartwork.translate('import_template_dialog_message') + '<br>' + madxartwork.translate('import_template_dialog_message_attention'),
      strings: {
        confirm: madxartwork.translate('yes'),
        cancel: madxartwork.translate('no')
      }
    });
  },
...
TemplateLibraryTemplateView = Marionette.ItemView.extend({
  className: function className() {
    var classes = 'madxartwork-template-library-template',
        source = this.model.get('source');
    classes += ' madxartwork-template-library-template-' + source;

    if ('remote' === source) {
      classes += ' madxartwork-template-library-template-' + this.model.get('type');
    }

    if (this.model.get('isPro')) {
      classes += ' madxartwork-template-library-pro-template';
    }
  ...
  ui: function ui() {
    return {
      previewButton: '.madxartwork-template-library-template-preview'
    };
  },
  events: function events() {
    return {
...
module.exports = Marionette.CompositeView.extend({
  ...
  addChildElement: function addChildElement(data, options) {
    ...
    if (options.trigger) {
      madxartwork.channels.data.trigger(options.trigger.beforeAdd, newItem);
    }
    ...
    if (options.trigger) {
      madxartwork.channels.data.trigger(options.trigger.afterAdd, newItem);
    }
  ...
  cloneItem: function cloneItem(item) {
    ...
    item.id = madxartwork.helpers.getUniqueID();
  ...
  onChildviewRequestPaste: function onChildviewRequestPaste(childView) {
    ...
    var elements = madxartworkCommon.storage.get('transfer').elements,
        index = self.collection.indexOf(childView.model);
    madxartwork.channels.data.trigger('element:before:add', elements[0]);
    ...
    madxartwork.channels.data.trigger('element:after:add', elements[0]);
  }
});
...
var _inline = _interopRequireDefault(__webpack_require__(374));

var BaseElementView = __webpack_require__(200),
    SectionView;

SectionView = BaseElementView.extend({
  template: Marionette.TemplateCache.get('#tmpl-madxartwork-section-content'),
  addSectionView: null,
  className: function className() {
    var classes = BaseElementView.prototype.className.apply(this, arguments),
        type = this.isInner() ? 'inner' : 'top';
    return classes + ' madxartwork-section madxartwork-' + type + '-section';
  },
  tagName: function tagName() {
    return this.model.getSetting('html_tag') || 'section';
  },
  childViewContainer: '> .madxartwork-container > .madxartwork-row',
  behaviors: function behaviors() {
    ...
    return madxartwork.hooks.applyFilters('elements/section/behaviors', behaviors, this);
  },
  errors: {
  ...
  getEditButtons: function getEditButtons() {
    var elementData = madxartwork.getElementData(this.model),
        editTools = {};

    if (!this.isInner()) {
      editTools.add = {
        title: madxartwork.translate('add_element', [elementData.title]),
        icon: 'plus'
      };
    }

    editTools.edit = {
      title: madxartwork.translate('edit_element', [elementData.title]),
      icon: 'handle'
    };

    if (madxartwork.config.editButtons) {
      editTools.duplicate = {
        title: madxartwork.translate('duplicate_element', [elementData.title]),
        icon: 'clone'
      };
    }

    editTools.remove = {
      title: madxartwork.translate('delete_element', [elementData.title]),
      icon: 'close'
    };
    return editTools;
  },
  ...
  getContextMenuGroups: function getContextMenuGroups() {
    ...
    groups.splice(transferGroupIndex + 1, 0, {
      ...
      actions: [{
        name: 'save',
        title: madxartwork.translate('save_as_block'),
  ...
  getSortableOptions: function getSortableOptions() {
    var sectionConnectClass = this.isInner() ? '.madxartwork-inner-section' : '.madxartwork-top-section';
    return {
      connectWith: sectionConnectClass + ' > .madxartwork-container > .madxartwork-row',
      handle: '> .madxartwork-element-overlay .madxartwork-editor-element-edit',
      items: '> .madxartwork-column',
  ...
  setStructure: function setStructure(structure) {
    var parsedStructure = madxartwork.presetsFactory.getParsedStructure(structure);
  ...
  redefineLayout: function redefineLayout() {
    var preset = madxartwork.presetsFactory.getPresetByStructure(this.getStructure());
  ...
  _checkIsFull: function _checkIsFull() {
    this.$el.toggleClass('madxartwork-section-filled', this.isCollectionFilled());
  },
  ...
  showChildrenPercentsTooltip: function showChildrenPercentsTooltip(columnView, nextColumnView) {
    ...
    columnView.ui.percentsTooltip.attr('data-side', madxartworkCommon.config.isRTL ? 'right' : 'left');
    nextColumnView.ui.percentsTooltip.show();
    nextColumnView.ui.percentsTooltip.attr('data-side', madxartworkCommon.config.isRTL ? 'left' : 'right');
  },
  ...
  onChildviewRequestResizeStart: function onChildviewRequestResizeStart(columnView) {
    ...
    madxartwork.helpers.disableElementEvents($iframes);
  },
  onChildviewRequestResizeStop: function onChildviewRequestResizeStop(columnView) {
    ...
    var $iframes = columnView.$el.find('iframe').add(nextColumnView.$el.find('iframe'));
    madxartwork.helpers.enableElementEvents($iframes);
  },
...
function (_Marionette$ItemView) {
  ...
  (0, _createClass2.default)(AddSectionBase, [{
    ...
    value: function template() {
      return Marionette.TemplateCache.get('#tmpl-madxartwork-add-section');
    }
  }, {
    key: "attributes",
    ...
    value: function ui() {
      return {
        addNewSection: '.madxartwork-add-new-section',
        closeButton: '.madxartwork-add-section-close',
        addSectionButton: '.madxartwork-add-section-button',
        addTemplateButton: '.madxartwork-add-template-button',
        selectPreset: '.madxartwork-select-preset',
        presets: '.madxartwork-preset'
      };
    }
  }, {
    key: "events",
    ...
    key: "className",
    value: function className() {
      return 'madxartwork-add-section madxartwork-visible-desktop';
    }
  }, {
    key: "addSection",
    value: function addSection(properties, options) {
      return madxartwork.getPreviewView().addChildElement(properties, jQuery.extend({}, this.options, options));
    }
  }, {
    key: "setView",
    ...
    value: function getContextMenuGroups() {
      var hasContent = function hasContent() {
        return madxartwork.elements.length > 0;
      };
      ...
      return [{
        ...
        actions: [{
          ...
          title: madxartwork.translate('copy_all_content'),
          callback: this.copy.bind(this),
          isEnabled: hasContent
        }, {
          name: 'delete_all_content',
          title: madxartwork.translate('delete_all_content'),
          callback: madxartwork.clearPage.bind(madxartwork),
          isEnabled: hasContent
        }]
      }];
    }
    ...
    key: "copy",
    value: function copy() {
      madxartwork.getPreviewView().copy();
    }
  }, {
    key: "paste",
    value: function paste() {
      madxartwork.getPreviewView().paste(this.getOption('at'));
    }
  }, {
    key: "isPasteEnabled",
    ...
    value: function onRender() {
      this.$el.html5Droppable({
        axis: ['vertical'],
        groups: ['madxartwork-element'],
        placeholder: false,
        currentElementClass: 'madxartwork-html5dnd-current-element',
        hasDraggingOnChildClass: 'madxartwork-dragging-on-child',
    ...
    value: function onPresetSelected(event) {
      ...
      var selectedStructure = event.currentTarget.dataset.structure,
          parsedStructure = madxartwork.presetsFactory.getParsedStructure(selectedStructure),
          elements = [];
      var loopIndex;
      ...
      madxartwork.channels.data.trigger('element:before:add', {
        elType: 'section'
      });
      ...
      madxartwork.channels.data.trigger('element:after:add');
    }
  }, {
    key: "onDropping",
    value: function onDropping() {
      madxartwork.channels.data.trigger('section:before:drop');
      ...
      madxartwork.channels.data.trigger('section:after:drop');
    }
  }]);
...
ControlBoxShadowItemView = ControlMultipleBaseItemView.extend({
  ui: function ui() {
    ...
    ui.colors = '.madxartwork-shadow-color-picker';
    return ui;
  },
  initSliders: function initSliders() {
    ...
    this.ui.sliders.each(function (index, slider) {
      var $input = jQuery(slider).next('.madxartwork-slider-input').find('input');
  ...
  initColors: function initColors() {
    var self = this;
    madxartwork.helpers.wpColorPicker(this.ui.colors, {
...
RepeaterRowView = Marionette.CompositeView.extend({
  template: Marionette.TemplateCache.get('#tmpl-madxartwork-repeater-row'),
  className: 'madxartwork-repeater-fields',
  ui: {
    duplicateButton: '.madxartwork-repeater-tool-duplicate',
    editButton: '.madxartwork-repeater-tool-edit',
    removeButton: '.madxartwork-repeater-tool-remove',
    itemTitle: '.madxartwork-repeater-row-item-title'
  },
  behaviors: {
    HandleInnerTabs: {
  ...
  childViewContainer: '.madxartwork-repeater-row-controls',
  getChildView: function getChildView(item) {
    var controlType = item.get('type');
    return madxartwork.getControlView(controlType);
  },
  ...
  setTitle: function setTitle() {
    ...
    if (!title) {
      title = madxartwork.translate('Item #%s', [this.getOption('itemIndex')]);
    }
  ...
  initialize: function initialize(options) {
    ...
    this.collection = new Backbone.Collection(_.values(madxartwork.mergeControlsSettings(options.controlFields)));
  },
  onRender: function onRender() {
    this.setTitle();
  },
  ...
  onChildviewResponsiveSwitcherClick: function onChildviewResponsiveSwitcherClick(childView, device) {
    if ('desktop' === device) {
      madxartwork.getPanelView().getCurrentPageView().$el.toggleClass('madxartwork-responsive-switchers-open');
    }
  }
});
...
var ColumnSettingsModel = __webpack_require__(410),
    ElementModel;
...
ElementModel = Backbone.Model.extend({
  ...
  initSettings: function initSettings() {
    var elType = this.get('elType'),
      ...
      column: ColumnSettingsModel
        ...
        SettingsModel = settingModels[elType] || madxartworkModules.editor.elements.models.BaseSettings;

    if (jQuery.isEmptyObject(settings)) {
      settings = madxartworkCommon.helpers.cloneObject(settings);
    }
    ...
    settings = new SettingsModel(settings, {
      controls: madxartwork.getElementControls(this)
    });
    this.set('settings', settings);
    madxartworkFrontend.config.elements.data[this.cid] = settings;
  },
  initEditSettings: function initEditSettings() {
    ...
    madxartworkFrontend.config.elements.editSettings[this.cid] = editSettings;
  },
  ...
  getDefaultTitle: function getDefaultTitle() {
    return madxartwork.getElementData(this).title;
  },
  ...
  getIcon: function getIcon() {
    return madxartwork.getElementData(this).icon;
  },
  createRemoteRenderRequest: function createRemoteRenderRequest() {
    var data = this.toJSON();
    return madxartworkCommon.ajax.addRequest('render_widget', {
      unique_id: this.cid,
      data: {
        data: data
      },
  ...
  clone: function clone() {
    var newModel = new this.constructor(madxartworkCommon.helpers.cloneObject(this.attributes));
    newModel.set('id', madxartwork.helpers.getUniqueID());
...
var ElementsCollection = Backbone.Collection.extend({
  ...
  model: function model(attrs, options) {
    ...
    if (attrs.elType) {
      ModelClass = madxartwork.hooks.applyFilters('element/model', ElementModel, attrs);
    }
...
module.exports = Marionette.ItemView.extend({
  template: '#tmpl-madxartwork-panel-global',
  id: 'madxartwork-panel-global',
  initialize: function initialize() {
    madxartwork.getPanelView().getCurrentPageView().search.reset();
  }
});
...
module.exports = Marionette.ItemView.extend({
  template: '#tmpl-madxartwork-element-library-element',
  className: 'madxartwork-element-wrapper',
  ui: {
    element: '.madxartwork-element'
  },
  onRender: function onRender() {
    var _this = this;
    ...
    this.ui.element.html5Draggable({
      onDragStart: function onDragStart() {
        madxartwork.channels.panelElements.reply('element:selected', _this).trigger('element:drag:start');
      },
      onDragEnd: function onDragEnd() {
        madxartwork.channels.panelElements.trigger('element:drag:end');
      },
      groups: ['madxartwork-element']
    });
  }
...
PanelSchemeColorsView = PanelSchemeBaseView.extend({
  ui: function ui() {
    ...
    ui.systemSchemes = '.madxartwork-panel-scheme-color-system-scheme';
    return ui;
  },
  events: function events() {
  ...
  onSystemSchemeClick: function onSystemSchemeClick(event) {
    var $schemeClicked = jQuery(event.currentTarget),
        ...
        scheme = madxartwork.config.system_schemes[this.getType()][schemeName].items;
...
var childViewTypes = {
  color: __webpack_require__(441),
  typography: __webpack_require__(442)
},
    PanelSchemeBaseView;
PanelSchemeBaseView = Marionette.CompositeView.extend({
  id: function id() {
    return 'madxartwork-panel-scheme-' + this.getType();
  },
  className: function className() {
    return 'madxartwork-panel-scheme madxartwork-panel-scheme-' + this.getUIType();
  },
  childViewContainer: '.madxartwork-panel-scheme-items',
  getTemplate: function getTemplate() {
    return Marionette.TemplateCache.get('#tmpl-madxartwork-panel-schemes-' + this.getType());
  },
  ...
  ui: function ui() {
    return {
      saveButton: '.madxartwork-panel-scheme-save .madxartwork-button',
      discardButton: '.madxartwork-panel-scheme-discard .madxartwork-button',
      resetButton: '.madxartwork-panel-scheme-reset .madxartwork-button'
    };
  },
  events: function events() {
    return {
  ...
  getScheme: function getScheme() {
    return madxartwork.schemes.getScheme(this.getType());
  },
  ...
  discardScheme: function discardScheme() {
    madxartwork.schemes.resetSchemes(this.getType());
    this.onSchemeChange();
  ...
  setSchemeValue: function setSchemeValue(key, value) {
    madxartwork.schemes.setSchemeValue(this.getType(), key, value);
    this.onSchemeChange();
  },
  saveScheme: function saveScheme() {
    madxartwork.schemes.saveScheme(this.getType());
    this.ui.saveButton.prop('disabled', true);
    this.resetScheme();
  ...
  setDefaultScheme: function setDefaultScheme() {
    var defaultScheme = madxartwork.config.default_schemes[this.getType()].items;
  ...
  onSchemeChange: function onSchemeChange() {
    madxartwork.schemes.printSchemesStyle();
  },
...
PanelSchemeItemView = Marionette.ItemView.extend({
  getTemplate: function getTemplate() {
    return Marionette.TemplateCache.get('#tmpl-madxartwork-panel-scheme-' + this.getUIType() + '-item');
  },
  className: function className() {
    return 'madxartwork-panel-scheme-item';
  }
});
 ...
 */
(function ($) {
  $.fn.madxartworkSerializeObject = function () {
...
var _component = _interopRequireDefault(__webpack_require__(329));

/* global madxartworkConfig */
var App = Marionette.Application.extend({
  loaded: false,
  previewLoadedOnce: false,
  helpers: __webpack_require__(330),
  imagesManager: __webpack_require__(337),
  debug: __webpack_require__(338),
  schemes: __webpack_require__(339),
  presetsFactory: __webpack_require__(340),
  templates: __webpack_require__(341),
  // TODO: BC Since 2.3.0
  ajax: madxartworkCommon.ajax,
  conditions: __webpack_require__(357),
  history: __webpack_require__(358),
  channels: {
    editor: Backbone.Radio.channel('madxartwork:editor'),
    data: Backbone.Radio.channel('madxartwork:data'),
    panelElements: Backbone.Radio.channel('madxartwork:panelElements'),
    dataEditMode: Backbone.Radio.channel('madxartwork:editmode'),
    deviceMode: Backbone.Radio.channel('madxartwork:deviceMode'),
    templates: Backbone.Radio.channel('madxartwork:templates')
  },

  /**
   * Exporting modules that can be used externally
   * @TODO: All of the following entries should move to `madxartworkModules.editor`
   */
  modules: {
    // TODO: Deprecated alias since 2.3.0
    get Module() {
      madxartworkCommon.helpers.deprecatedMethod('madxartwork.modules.Module', '2.3.0', 'madxartworkModules.Module');
      return madxartworkModules.Module;
    },
    ...
    components: {
      templateLibrary: {
        views: {
          ...
          get BaseModalLayout() {
            madxartworkCommon.helpers.deprecatedMethod('madxartwork.modules.components.templateLibrary.views.BaseModalLayout', '2.4.0', 'madxartworkModules.common.views.modal.Layout');
            return madxartworkModules.common.views.modal.Layout;
          }
      ...
      saver: {
        behaviors: {
          FooterSaver: __webpack_require__(382)
        }
      }
    },
    controls: {
      ...
      Color: __webpack_require__(385),
      Date_time: _dateTime.default,
      Dimensions: __webpack_require__(386),
      Exit_animation: __webpack_require__(182),
      Font: __webpack_require__(389),
      Gallery: __webpack_require__(390),
      Hidden: __webpack_require__(391),
      Hover_animation: __webpack_require__(182),
      Icon: __webpack_require__(392),
      Icons: __webpack_require__(393),
      Image_dimensions: __webpack_require__(394),
      Media: __webpack_require__(395),
      Number: __webpack_require__(396),
      Order: __webpack_require__(398),
      Popover_toggle: __webpack_require__(399),
      Repeater: __webpack_require__(400),
      RepeaterRow: __webpack_require__(254),
      Section: __webpack_require__(401),
      Select: __webpack_require__(402),
      Select2: __webpack_require__(182),
      Slider: __webpack_require__(403),
      Structure: __webpack_require__(404),
      Switcher: __webpack_require__(405),
      Tab: __webpack_require__(406),
      Text_shadow: __webpack_require__(251),
      Url: __webpack_require__(407),
      Wp_widget: __webpack_require__(408),
      Wysiwyg: __webpack_require__(409)
    },
    elements: {
      models: {
        // TODO: Deprecated alias since 2.4.0
        get BaseSettings() {
          madxartworkCommon.helpers.deprecatedMethod('madxartwork.modules.elements.models.BaseSettings', '2.4.0', 'madxartworkModules.editor.elements.models.BaseSettings');
          return madxartworkModules.editor.elements.models.BaseSettings;
        },

        Element: __webpack_require__(255)
      },
      views: {
        Widget: __webpack_require__(411)
      }
    },
    layouts: {
      panel: {
        pages: {
          elements: {
            views: {
              ...
              Elements: __webpack_require__(415)
            }
          },
          menu: {
            Menu: __webpack_require__(416)
          }
        }
      }
    },
    views: {
      // TODO: Deprecated alias since 2.4.0
      get ControlsStack() {
        madxartworkCommon.helpers.deprecatedMethod('madxartwork.modules.views.ControlsStack', '2.4.0', 'madxartworkModules.editor.views.ControlsStack');
        return madxartworkModules.editor.views.ControlsStack;
      }
  ...
  backgroundClickListeners: {
    popover: {
      element: '.madxartwork-controls-popover',
      ignore: '.madxartwork-control-popover-toggle-toggle, .madxartwork-control-popover-toggle-toggle-label, .select2-container'
    },
    tagsList: {
      element: '.madxartwork-tags-list',
      ignore: '.madxartwork-control-dynamic-switcher'
    },
    panelFooterSubMenus: {
      element: '.madxartwork-panel-footer-tool.madxartwork-toggle-state',
      ignore: '.madxartwork-panel-footer-tool.madxartwork-toggle-state, #madxartwork-panel-saver-button-publish-label',
      callback: function callback($elementsToHide) {
        $elementsToHide.removeClass('madxartwork-open');
      }
    }
  },
  ...
  addControlView: function addControlView(controlID, ControlView) {
    this.modules.controls[madxartworkCommon.helpers.upperCaseWords(controlID)] = ControlView;
  },
  ...
  getElementData: function getElementData(model) {
    ...
    var elementConfig = madxartworkCommon.helpers.cloneObject(this.config.elements[elType]);
  ...
  getControlView: function getControlView(controlID) {
    var capitalizedControlName = madxartworkCommon.helpers.upperCaseWords(controlID),
  ...
  initComponents: function initComponents() {
    var EventManager = __webpack_require__(229),
        DynamicTags = __webpack_require__(419),
        Settings = __webpack_require__(421),
        Saver = __webpack_require__(427),
        Notifications = __webpack_require__(428);
  ...
  initDialogsManager: function initDialogsManager() {
    this.dialogsManager = madxartworkCommon.dialogsManager;
  },
  ...
  initPreview: function initPreview() {
    var $ = jQuery;
    this.$previewWrapper = $('#madxartwork-preview');
    this.$previewResponsiveWrapper = $('#madxartwork-preview-responsive-wrapper');
    var previewIframeId = 'madxartwork-preview-iframe'; // Make sure the iFrame does not exist.
  ...
  initFrontend: function initFrontend() {
    ...
    window.madxartworkFrontend = frontendWindow.madxartworkFrontend;
    frontendWindow.madxartwork = this;
    madxartworkFrontend.init();
  ...
  initClearPageDialog: function initClearPageDialog() {
    ...
    self.getClearPageDialog = function () {
      ...
      dialog = madxartworkCommon.dialogsManager.createWidget('confirm', {
        id: 'madxartwork-clear-page-dialog',
        headerMessage: madxartwork.translate('clear_page'),
        message: madxartwork.translate('dialog_confirm_clear_page'),
        position: {
          my: 'center center',
          at: 'center center'
        },
        strings: {
          confirm: madxartwork.translate('delete'),
          cancel: madxartwork.translate('cancel')
        },
  ...
  getCurrentElement: function getCurrentElement() {
    var isPreview = -1 !== ['BODY', 'IFRAME'].indexOf(document.activeElement.tagName) && 'BODY' === madxartworkFrontend.elements.window.document.activeElement.tagName;

    if (!isPreview) {
      return false;
    }

    var targetElement = madxartwork.channels.editor.request('contextMenu:targetView');
    ...
    if (!targetElement) {
      targetElement = madxartwork.getPreviewView();
    }
  ...
  setAjax: function setAjax() {
    madxartworkCommon.ajax.addRequestConstant('editor_post_id', this.config.document.id);
    madxartworkCommon.ajax.on('request:unhandledError', function (xmlHttpRequest) {
      madxartwork.notifications.showToast({
        message: madxartwork.createAjaxErrorMessage(xmlHttpRequest)
      });
    });
  },
  ...
  preventClicksInsideEditor: function preventClicksInsideEditor() {
    ...
    this.$previewContents.on('click', function (event) {
      var $target = jQuery(event.target),
          editMode = madxartwork.channels.dataEditMode.request('activeMode'),
          isClickInsidemadxartwork = !!$target.closest('#madxartwork, .pen-menu').length,
          isTargetInsideDocument = this.contains($target[0]);

      if (isClickInsidemadxartwork && 'edit' === editMode || !isTargetInsideDocument) {
        return;
      }

      if ($target.closest('a:not(.madxartwork-clickable)').length) {
        event.preventDefault();
      }

      if (!isClickInsidemadxartwork) {
        $e.route('panel/elements/categories');
      }
    });
  },
  ...
  showFatalErrorDialog: function showFatalErrorDialog(options) {
    var defaultOptions = {
      id: 'madxartwork-fatal-error-dialog',
      headerMessage: '',
      message: '',
      position: {
        my: 'center center',
    ...
    madxartworkCommon.dialogsManager.createWidget('confirm', options).show();
  },
  showFlexBoxAttentionDialog: function showFlexBoxAttentionDialog() {
    var _this2 = this;

    var introduction = new madxartworkModules.editor.utils.Introduction({
      introductionKey: 'flexbox',
      dialogType: 'confirm',
      dialogOptions: {
        id: 'madxartwork-flexbox-attention-dialog',
  ...
  checkPageStatus: function checkPageStatus() {
    if (madxartwork.config.current_revision_id !== madxartwork.config.document.id) {
      this.notifications.showToast({
        message: this.translate('working_on_draft_notification'),
        buttons: [{
          name: 'view_revisions',
          text: madxartwork.translate('view_all_revisions'),
          callback: function callback() {
  ...
  enterPreviewMode: function enterPreviewMode(hidePanel) {
    var $elements = madxartworkFrontend.elements.$body;

    if (hidePanel) {
      $elements = $elements.add(madxartworkCommon.elements.$body);
    }

    $elements.removeClass('madxartwork-editor-active').addClass('madxartwork-editor-preview');
    this.$previewmadxartworkEl.removeClass('madxartwork-edit-area-active').addClass('madxartwork-edit-area-preview');

    if (hidePanel) {
      // Handle panel resize
      this.$previewWrapper.css(madxartworkCommon.config.isRTL ? 'right' : 'left', '');
      this.panel.$el.css('width', '');
    }
  },
  exitPreviewMode: function exitPreviewMode() {
    madxartworkFrontend.elements.$body.add(madxartworkCommon.elements.$body).removeClass('madxartwork-editor-preview').addClass('madxartwork-editor-active');
    this.$previewmadxartworkEl.removeClass('madxartwork-edit-area-preview').addClass('madxartwork-edit-area-active');
  },
  changeEditMode: function changeEditMode(newMode) {
    var dataEditMode = madxartwork.channels.dataEditMode,
  ...
  reloadPreview: function reloadPreview() {
    jQuery('#madxartwork-preview-loading').show();
  ...
  changeDeviceMode: function changeDeviceMode(newDeviceMode) {
    ...
    madxartworkCommon.elements.$body.removeClass('madxartwork-device-' + oldDeviceMode).addClass('madxartwork-device-' + newDeviceMode);
  ...
  translate: function translate(stringKey, templateArgs, i18nStack) {
    ...
    return madxartworkCommon.translate(stringKey, null, templateArgs, i18nStack);
  },
  logSite: function logSite() {
    ...
    if (_environment.default.firefox) {
      ...
      text += '%c00';
      style = 'font-size: 22px; background-image: url("' + madxartworkCommon.config.urls.assets + 'images/logo-icon.png"); color: transparent; background-repeat: no-repeat';
    }

    setTimeout(console.log.bind(console, text, style)); // eslint-disable-line

    text = '%cLove using madxartwork? Join our growing community of madxartwork developers: %chttps://github.com/madxartwork/madxartwork';
  ...
  requestWidgetsConfig: function requestWidgetsConfig() {
    ...
    madxartworkCommon.ajax.addRequest('get_widgets_config', {
      data: {
      ...
      success: function success(data) {
        ...
        madxartworkCommon.elements.$body.addClass('madxartwork-controls-ready');
      }
    });
  },
  onStart: function onStart() {
    NProgress.start();
    NProgress.inc(0.2);
    this.config = madxartworkConfig;
    Backbone.Radio.DEBUG = false;
    Backbone.Radio.tuneIn('madxartwork');
    this.initComponents();
    ...
    madxartworkCommon.elements.$window.trigger('madxartwork:init');
    this.initPreview();
    this.logSite();
  },
  onPreviewLoaded: function onPreviewLoaded() {
    ...
    if (!previewWindow.madxartworkFrontend) {
      this.onPreviewLoadingError();
      return;
    }

    this.$previewContents = this.$preview.contents();
    this.$previewmadxartworkEl = this.$previewContents.find('#madxartwork');

    if (!this.$previewmadxartworkEl.length) {
    ...
    var iframeRegion = new Marionette.Region({
      ...
      el: this.$previewmadxartworkEl[0]
    });
    this.schemes.init();
    this.schemes.printSchemesStyle();
    this.preventClicksInsideEditor();
    this.addBackgroundClickArea(madxartworkFrontend.elements.window.document);
    ...
    var Preview = __webpack_require__(448);

    this.sections.show(new Preview({
      model: this.elementsModel
    }));
    this.$previewContents.children().addClass('madxartwork-html');
    var $frontendBody = madxartworkFrontend.elements.$body;
    $frontendBody.addClass('madxartwork-editor-active');

    if (!madxartwork.userCan('design')) {
      $frontendBody.addClass('madxartwork-editor-content-only');
    }

    this.changeDeviceMode(this._defaultDeviceMode);
    jQuery('#madxartwork-loading, #madxartwork-preview-loading').fadeOut(600);

    _.defer(function () {
      madxartworkFrontend.elements.window.jQuery.holdReady(false);
    });

    this.enqueueTypographyFonts();
    this.onEditModeSwitched();
    $e.shortcuts.bindListener(madxartworkFrontend.elements.$window);
  ...
  onEnvNotCompatible: function onEnvNotCompatible() {
    this.showFatalErrorDialog({
      ...
      strings: {
        confirm: madxartwork.translate('proceed_anyway')
      },
      hide: {
        onButtonClick: true
      },
  ...
  onPreviewLoadingError: function onPreviewLoadingError() {
    ...
    var debugUrl = self.config.document.urls.preview + '&preview-debug',
        ...
        previewDebugLink = '<div id="madxartwork-preview-debug-link-text"><a href="' + debugUrl + '" target="_blank">' + previewDebugLinkText + '</a></div>',
        debugData = madxartwork.config.preview.debug_data,
        dialogOptions = {
      className: 'madxartwork-preview-loading-error',
    ...
    jQuery.get(debugUrl, function () {
      ...
      self.showFatalErrorDialog({
        ...
        onConfirm: function onConfirm() {
          var url = 500 <= response.status ? madxartwork.config.preview.help_preview_http_error_500_url : madxartwork.config.preview.help_preview_http_error_url;
          open(url, '_blank');
        }
      });
    });
  ...
  onPreviewElNotFound: function onPreviewElNotFound() {
    var args = this.$preview[0].contentWindow.madxartworkPreviewErrorArgs;

    if (!args) {
      args = {
        headerMessage: this.translate('preview_el_not_found_header'),
        message: this.translate('preview_el_not_found_message'),
        confirmURL: madxartwork.config.help_the_content_url
      };
    }
...
window.madxartwork = new App();

if (-1 === location.href.search('madxartwork_TESTS=1')) {
  madxartwork.start();
}

module.exports = madxartwork;
...
function () {
  function Heartbeat() {
    ...
    jQuery(document).on({
      'heartbeat-send': function heartbeatSend(event, data) {
        data.madxartwork_post_lock = {
          post_ID: madxartwork.config.document.id
        };
      },
      'heartbeat-tick': function heartbeatTick(event, response) {
        if (response.locked_user) {
          if (madxartwork.saver.isEditorChanged()) {
            madxartwork.saver.saveEditor({
              status: 'autosave'
            });
          }
        ...
        madxartworkCommon.ajax.addRequestConstant('_nonce', response.madxartworkNonce);
      },
      'heartbeat-tick.wp-refresh-nonces': function heartbeatTickWpRefreshNonces(event, response) {
        var nonces = response['madxartwork-refresh-nonces'];

        if (nonces) {
          if (nonces.heartbeatNonce) {
            madxartworkCommon.ajax.addRequestConstant('_nonce', nonces.madxartworkNonce);
          }
    ...
    if (madxartwork.config.locked_user) {
      this.showLockMessage(madxartwork.config.locked_user);
    }
  }
  ...
  (0, _createClass2.default)(Heartbeat, [{
    key: "initModal",
    value: function initModal() {
      var modal = madxartworkCommon.dialogsManager.createWidget('lightbox', {
        headerMessage: madxartwork.translate('take_over')
      });
      modal.addButton({
        name: 'go_back',
        text: madxartwork.translate('go_back'),
        callback: function callback() {
          parent.history.go(-1);
        }
      });
      modal.addButton({
        name: 'take_over',
        text: madxartwork.translate('take_over'),
        callback: function callback() {
          wp.heartbeat.enqueue('madxartwork_force_post_lock', true);
    ...
    value: function showLockMessage(lockedUser) {
      ...
      modal.setMessage(madxartwork.translate('dialog_user_taken_over', [lockedUser])).show();
    }
  }]);
  return Heartbeat;
}();
...
function (_BaseRegion) {
  ...
  function _default(options) {
    ...
    _this.indicators = {
      customPosition: {
        title: madxartwork.translate('custom_positioning'),
        icon: 'cursor-move',
    ...
    _this.listenTo(madxartwork.channels.dataEditMode, 'switch', _this.onEditModeSwitched);

    madxartwork.on('navigator:init', function () {
  ...
  (0, _createClass2.default)(_default, [{
    ...
    value: function getDraggableOptions() {
      return {
        iframeFix: true,
        handle: '#madxartwork-navigator__header',
    ...
    value: function getResizableOptions() {
      ...
      return {
        ...
        start: function start() {
          madxartwork.$previewWrapper.addClass('ui-resizable-resizing');
        },
        stop: function stop() {
          madxartwork.$previewWrapper.removeClass('ui-resizable-resizing');

          if (_this2.isDocked) {
            _this2.storage.size.width = madxartwork.helpers.getElementInlineStyle(_this2.$el, ['width']).width;
            madxartworkCommon.storage.set('navigator', _this2.storage);
          } else {
            _this2.saveSize();
          }
        }
    ...
    value: function open(model) {
      ...
      this.ensurePosition();
      madxartworkCommon.elements.$window.on('resize', this.ensurePosition);
    }
  }, {
    key: "close",
    value: function close(silent) {
      ...
      madxartworkCommon.elements.$window.off('resize', this.ensurePosition);
    }
  }, {
    key: "isOpen",
    ...
    key: "dock",
    value: function dock() {
      madxartworkCommon.elements.$body.addClass('madxartwork-navigator-docked');
      var side = madxartworkCommon.config.isRTL ? 'left' : 'right',
      ...
      madxartwork.$previewWrapper.css(side, this.storage.size.width);
      this.$el.resizable('destroy');
      resizableOptions.handles = madxartworkCommon.config.isRTL ? 'e' : 'w';

      resizableOptions.resize = function (event, ui) {
        madxartwork.$previewWrapper.css(side, ui.size.width);
      };
    ...
    value: function undock(silent) {
      madxartworkCommon.elements.$body.removeClass('madxartwork-navigator-docked');
      madxartwork.$previewWrapper.css(madxartworkCommon.config.isRTL ? 'left' : 'right', '');
      this.setSize();
      this.$el.resizable('destroy');
      this.$el.resizable(this.getResizableOptions());
      this.isDocked = false;
    ...
    value: function onDrag(event, ui) {
      ...
      if (madxartworkCommon.config.isRTL) {
        if (isOutOfRight) {
      ...
      madxartworkCommon.elements.$body.toggleClass('madxartwork-navigator--dock-hint', madxartworkCommon.config.isRTL ? isOutOfLeft : isOutOfRight);
    }
  }, {
    key: "onDragStop",
    value: function onDragStop(event, ui) {
      ...
      madxartworkCommon.elements.$body.removeClass('madxartwork-navigator--dock-hint');
    }
  }, {
...
var _default =
/*#__PURE__*/
function (_madxartworkModules$com) {
  (0, _inherits2.default)(_default, _madxartworkModules$com);
      ...
      return {
        toggle: {
          ...
          dependency: function dependency() {
            return 'edit' === madxartwork.channels.dataEditMode.request('activeMode');
          }
        }
      };
    }
  ...
  return _default;
}(madxartworkModules.common.Component);
...
function (_Marionette$LayoutVie) {
  ...
  (0, _createClass2.default)(_default, [{
    key: "getTemplate",
    value: function getTemplate() {
      return '#tmpl-madxartwork-navigator';
    }
  }, {
    key: "id",
    value: function id() {
      return 'madxartwork-navigator__inner';
    }
  }, {
    key: "ui",
    value: function ui() {
      return {
        toggleAll: '#madxartwork-navigator__toggle-all',
        close: '#madxartwork-navigator__close'
      };
    }
  }, {
    key: "events",
    ...
    value: function regions() {
      return {
        elements: '#madxartwork-navigator__elements'
      };
    }
  }, {
    key: "toggleAll",
    value: function toggleAll() {
      var state = 'expand' === this.ui.toggleAll.data('madxartwork-action'),
          classes = ['eicon-collapse', 'eicon-expand'];
      this.ui.toggleAll.data('madxartwork-action', state ? 'collapse' : 'expand').removeClass(classes[+state]).addClass(classes[+!state]);
    ...
    value: function onShow() {
      this.elements.show(new _element.default({
        model: madxartwork.elementsModel
      }));
    }
  }, {
    key: "onCloseClick",
...
function (_Marionette$Composite) {
  ...
  (0, _createClass2.default)(_default, [{
    key: "getTemplate",
    value: function getTemplate() {
      return '#tmpl-madxartwork-navigator__elements';
    }
  }, {
    key: "ui",
    value: function ui() {
      return {
        item: '> .madxartwork-navigator__item',
        title: '> .madxartwork-navigator__item .madxartwork-navigator__element__title__text',
        toggle: '> .madxartwork-navigator__item > .madxartwork-navigator__element__toggle',
        toggleList: '> .madxartwork-navigator__item > .madxartwork-navigator__element__list-toggle',
        indicators: '> .madxartwork-navigator__item > .madxartwork-navigator__element__indicators',
        indicator: '> .madxartwork-navigator__item > .madxartwork-navigator__element__indicators > .madxartwork-navigator__element__indicator',
        elements: '> .madxartwork-navigator__elements'
      };
    }
  }, {
    ...
    value: function className() {
      ...
      var classes = 'madxartwork-navigator__element';

      if (elType) {
        classes += ' madxartwork-navigator__element-' + elType;
      }

      if (this.hasChildren()) {
        classes += ' madxartwork-navigator__element--has-children';
      }
    ...
    value: function initialize() {
      this.collection = this.model.get('elements');
      this.childViewContainer = '.madxartwork-navigator__elements';
    ...
    value: function toggleList(state, callback) {
      ...
      var isActive = this.ui.item.hasClass('madxartwork-active');

      if (isActive === state) {
        return;
      }

      this.ui.item.toggleClass('madxartwork-active', state);
    ...
    value: function toggleHiddenClass() {
      this.$el.toggleClass('madxartwork-navigator__element--hidden', !!this.model.get('hidden'));
    }
  }, {
    ...
    value: function dragShouldBeIgnored(draggedModel) {
      var childTypes = madxartwork.helpers.getElementChildType(this.model.get('elType')),
    ...
    value: function addEditingClass() {
      this.ui.item.addClass('madxartwork-editing');
    }
  }, {
    key: "removeEditingClass",
    value: function removeEditingClass() {
      this.ui.item.removeClass('madxartwork-editing');
    }
  }, {
    ...
    value: function enterTitleEditing() {
      ...
      madxartwork.addBackgroundClickListener('navigator', {
    ...
    value: function exitTitleEditing() {
      ...
      madxartwork.removeBackgroundClickListener('navigator');
    }
  }, {
    ...
    value: function activateSortable() {
      ...
      this.ui.elements.sortable({
        items: '> .madxartwork-navigator__element',
        placeholder: 'ui-sortable-placeholder',
        axis: 'y',
        forcePlaceholderSize: true,
        connectWith: '.madxartwork-navigator__element-' + this.model.get('elType') + ' ' + this.ui.elements.selector,
    ...
    value: function renderIndicators() {
      ...
      jQuery.each(madxartwork.navigator.indicators, function (indicatorName, indicatorSettings) {
        ...
        var $indicator = jQuery('<div>', {
          class: 'madxartwork-navigator__element__indicator',
    ...
    value: function onRender() {
      ...
      this.ui.item.css('padding-' + (madxartworkCommon.config.isRTL ? 'right' : 'left'), this.getIndent());
    ...
    value: function onSortStart(event, ui) {
      ...
      jQuery(ui.item).children('.madxartwork-navigator__item').trigger('click');
      madxartwork.navigator.getLayout().activateElementsMouseInteraction();
    }
  }, {
    key: "onSortStop",
    value: function onSortStop() {
      madxartwork.navigator.getLayout().deactivateElementsMouseInteraction();
    }
  }, {
    key: "onSortOver",
    ...
    value: function onMouseEnter(event) {
      ...
      event.stopPropagation();
      var dragShouldBeIgnored = this.recursiveChildAgreement('dragShouldBeIgnored', madxartwork.channels.data.request('dragging:model'));
    ...
    value: function onEditRequest() {
      ...
      madxartwork.navigator.getLayout().elements.currentView.recursiveChildInvoke('removeEditingClass');
      this.addEditingClass();
      madxartwork.helpers.scrollToView(this.$el, 400, madxartwork.navigator.getLayout().elements.$el);
    }
  }, {
    ...
    value: function onIndicatorClick(event) {
      ...
      setTimeout(function () {
        var editor = madxartwork.getPanelView().currentPageView,
...
function (_Marionette$ItemView) {
  ...
  (0, _createClass2.default)(_default, [{
    key: "getTemplate",
    value: function getTemplate() {
      return '#tmpl-madxartwork-navigator__elements--empty';
    }
  }, {
    key: "className",
    value: function className() {
      return 'madxartwork-empty-view';
    }
  }, {
    key: "onRendr",
    value: function onRendr() {
      this.$el.css('padding-' + (madxartworkCommon.config.isRTL ? 'right' : 'left'), this.getOption('indent'));
    }
  }]);
  return _default;
}(Marionette.ItemView);
...
function (_Marionette$ItemView) {
  ...
  (0, _createClass2.default)(_default, [{
    key: "getTemplate",
    value: function getTemplate() {
      return '#tmpl-madxartwork-navigator__root--empty';
    }
  }, {
    key: "className",
    value: function className() {
      return 'madxartwork-nerd-box';
    }
  }]);
  return _default;
}(Marionette.ItemView);
...
function (_madxartworkModules$com) {
  ...
  (0, _createClass2.default)(_default, [{
    ...
    value: function getLogoOptions() {
      return {
        title: madxartwork.translate('keyboard_shortcuts')
      };
    }
  }, {
    key: "initialize",
...
function (_Marionette$LayoutVie) {
  ...
  (0, _createClass2.default)(_default, [{
    ...
    key: "getTemplate",
    value: function getTemplate() {
      return '#tmpl-madxartwork-hotkeys';
    }
  }]);
  return _default;
}(Marionette.LayoutView);
...
module.exports = Marionette.Behavior.extend({
  ...
  ui: {
    tagArea: '.madxartwork-control-tag-area',
    dynamicSwitcher: '.madxartwork-control-dynamic-switcher'
  },
  events: {
  ...
  renderTools: function renderTools() {
    ...
    var $dynamicSwitcher = jQuery(Marionette.Renderer.render('#tmpl-madxartwork-control-dynamic-switcher'));

    if (this.view.model.get('label_block')) {
      this.ui.controlTitle.after($dynamicSwitcher);
      var $responsiveSwitchers = $dynamicSwitcher.next('.madxartwork-control-responsive-switchers');
  ...
  toggleDynamicClass: function toggleDynamicClass() {
    this.$el.toggleClass('madxartwork-control-dynamic-value', this.isDynamicMode());
  },
  ...
  createTagsList: function createTagsList() {
    var tags = _.groupBy(this.getOption('tags'), 'group'),
        groups = madxartwork.dynamicTags.getConfig('groups'),
        $tagsList = this.ui.tagsList = jQuery('<div>', {
      class: 'madxartwork-tags-list'
    }),
        $tagsListInner = jQuery('<div>', {
      class: 'madxartwork-tags-list__inner'
    });
    ...
    jQuery.each(groups, function (groupName) {
      ...
      var group = this,
        ...
        class: 'madxartwork-tags-list__group-title'
      }).text(group.title);
      $tagsListInner.append($groupTitle);
      groupTags.forEach(function (tag) {
        var $tag = jQuery('<div>', {
          class: 'madxartwork-tags-list__item'
        });
        $tag.text(tag.title).attr('data-tag-name', tag.name);
        $tagsListInner.append($tag);
      });
    });
    $tagsListInner.on('click', '.madxartwork-tags-list__item', this.onTagsListItemClick.bind(this));
    madxartworkCommon.elements.$body.append($tagsList);
  },
  ...
  toggleTagsList: function toggleTagsList() {
    ...
    var direction = madxartworkCommon.config.isRTL ? 'left' : 'right';
  ...
  setDefaultTagView: function setDefaultTagView() {
    var tagData = madxartwork.dynamicTags.tagTextToTagData(this.getDynamicValue());
  ...
  tagViewToTagText: function tagViewToTagText() {
    ...
    return madxartwork.dynamicTags.tagDataToTagText(tagView.getOption('id'), tagView.getOption('name'), tagView.model);
  },
  ...
  setDynamicValue: function setDynamicValue(value) {
    ...
    dynamicSettings = madxartworkCommon.helpers.cloneObject(dynamicSettings);
  ...
  onRender: function onRender() {
    this.$el.addClass('madxartwork-control-dynamic');
    this.renderTools();
    this.toggleDynamicClass();
  ...
  onTagsListItemClick: function onTagsListItemClick(event) {
    ...
    this.setTagView(madxartwork.helpers.getUniqueID(), $tag.data('tagName'), {});
...
module.exports = Marionette.ItemView.extend({
  className: 'madxartwork-dynamic-cover madxartwork-input-style',
  ...
  ui: {
    remove: '.madxartwork-dynamic-cover__remove'
  },
  events: function events() {
    var events = {
  ...
  getTemplate: function getTemplate() {
    var config = this.getTagConfig(),
        templateFunction = Marionette.TemplateCache.get('#tmpl-madxartwork-control-dynamic-cover'),
  ...
  getTagConfig: function getTagConfig() {
    return madxartwork.dynamicTags.getConfig('tags.' + this.getOption('name'));
  },
  initSettingsPopup: function initSettingsPopup() {
    var settingsPopupOptions = {
      className: 'madxartwork-tag-settings-popup',
      position: {
        my: 'left top+5',
        at: 'left bottom',
        of: this.$el,
    ...
    var settingsPopup = madxartworkCommon.dialogsManager.createWidget('buttons', settingsPopupOptions);
  ...
  initModel: function initModel() {
    this.model = new madxartworkModules.editor.elements.models.BaseSettings(this.getOption('settings'), {
...
module.exports = madxartworkModules.editor.views.ControlsStack.extend({
  ...
  getNamespaceArray: function getNamespaceArray() {
    var currentPageView = madxartwork.getPanelView().getCurrentPageView(),
...
module.exports = Marionette.ItemView.extend({
  className: 'madxartwork-tag-controls-stack-empty',
  template: '#tmpl-madxartwork-tag-controls-stack-empty'
});
...
function (_madxartworkModules$Vie) {
  ...
  (0, _createClass2.default)(_default, [{
    ...
    value: function getDefaultSettings() {
      return {
        selectors: {
          notice: '#madxartwork-notice-bar',
          close: '#madxartwork-notice-bar__close'
        }
      };
    }
  }, {
    ...
    value: function onCloseClick() {
      ...
      madxartworkCommon.ajax.addRequest('notice_bar_dismiss');
    }
  }]);
  return _default;
...
var _iconLibrary = _interopRequireDefault(__webpack_require__(327));

var _store = _interopRequireDefault(__webpack_require__(328));
...
function (_madxartworkModules$Mod) {
  ...
  (0, _createClass2.default)(_default, [{
    ...
    value: function onInit() {
      ...
      madxartwork.helpers.fetchFa4ToFa5Mapping();
      this.cache = {};
    }
  }, {
    key: "getLayout",
    value: function getLayout() {
      ...
      if (!this.layout) {
        ...
        layoutModal.addButton({
          name: 'insert_icon',
          text: madxartwork.translate('Insert'),
          classes: 'madxartwork-button madxartwork-button-success',
          callback: function callback() {
    ...
    value: function unMountIconManager() {
      var containerElement = document.querySelector('#madxartwork-icons-manager-modal .dialog-content');
    ...
    value: function loadIconLibraries() {
      if (!this.cache.loaded) {
        madxartwork.config.icons.libraries.forEach(function (library) {
          if ('all' === library.name) {
            return;
          }

          madxartwork.iconManager.library.initIconType(library);
        });
    ...
    value: function onPickerShow() {
      ...
      var selected = controlView.getControlValue(),
          icons = madxartwork.config.icons.libraries;
      ...
      icons.forEach(function (tab, index) {
        if (-1 === ['all', 'recommended'].indexOf(tab.name)) {
          madxartwork.iconManager.library.initIconType(tab, function (lib) {
            icons[index] = lib;
          });
        }
      ...
      iconManagerConfig.customIconsURL = madxartwork.config.customIconsURL;
...
function (_BaseModalLayout) {
  ...
  (0, _createClass2.default)(_default, [{
    ...
    value: function getModalOptions() {
      return {
        id: 'madxartwork-icons-manager-modal'
      };
    }
  }, {
    key: "getLogoOptions",
    value: function getLogoOptions() {
      return {
        title: madxartwork.translate('icon_library')
      };
    }
  }, {
    key: "initialize",
...
var _tab = _interopRequireDefault(__webpack_require__(310));

var _iconsGoPro = _interopRequireDefault(__webpack_require__(326));
...
function (_Component) {
  ...
  function IconsManager() {
    ...
    (0, _defineProperty2.default)((0, _assertThisInitialized2.default)(_this), "state", {
      ...
      iconTabs: madxartwork.config.icons.libraries,
    ...
    (0, _defineProperty2.default)((0, _assertThisInitialized2.default)(_this), "loadAllTabs", function () {
      ...
      icons.forEach(function (tabSettings) {
        ...
        madxartwork.iconManager.library.initIconType((0, _objectSpread2.default)({}, tabSettings), function (library) {
    ...
    (0, _defineProperty2.default)((0, _assertThisInitialized2.default)(_this), "getIconTabsLinks", function () {
      ...
      return _this.props.icons.map(function (tab) {
        ...
        var className = 'madxartwork-icons-manager__tab-link';

        if (isCurrentTab) {
          className += ' madxartwork-active';
        }
    ...
    (0, _defineProperty2.default)((0, _assertThisInitialized2.default)(_this), "getActiveTabIcons", function (activeTab) {
      ...
      if (!_this.state.loaded[activeTab]) {
        ...
        return madxartwork.iconManager.library.initIconType((0, _objectSpread2.default)({}, librarySettings[0]), function (library) {
          _this.cache[library.name] = library;
      ...
      return madxartwork.iconManager.store.getIcons(activeTab);
    });
    ...
    (0, _defineProperty2.default)((0, _assertThisInitialized2.default)(_this), "setSelected", function (selected) {
      madxartwork.iconManager.setSettings('selectedIcon', selected);
    ...
    (0, _defineProperty2.default)((0, _assertThisInitialized2.default)(_this), "render", function () {
      ...
      return _react.default.createElement(_react.Fragment, null, _react.default.createElement("div", {
        id: "madxartwork-icons-manager__sidebar",
        className: 'madxartwork-templates-modal__sidebar'
      }, _react.default.createElement("div", {
        id: "madxartwork-icons-manager__tab-links"
      }, _this.getIconTabsLinks(), _this.getUploadCustomButton(), _this.getIconTabsLinks(false))), _react.default.createElement("div", {
        id: "madxartwork-icons-manager__main",
        className: 'madxartwork-templates-modal__content'
        ...
        id: "madxartwork-icons-manager__tab__wrapper",
        ref: _this.scrollViewRef
      }, _react.default.createElement("div", {
        id: "madxartwork-icons-manager__tab__title"
      }, activeTab.label), _react.default.createElement("div", {
        id: "madxartwork-icons-manager__tab__content_wrapper"
  ...
  (0, _createClass2.default)(IconsManager, [{
    ...
    value: function getUploadCustomButton() {
      ...
      return _react.default.createElement("div", {
        id: "madxartwork-icons-manager__upload"
      }, _react.default.createElement("div", {
        id: "madxartwork-icons-manager__upload__title"
      }, madxartwork.translate('my_libraries')), _react.default.createElement("button", {
        id: "madxartwork-icons-manager__upload__button",
        className: "madxartwork-button madxartwork-button-default",
        onClick: onClick
      }, madxartwork.translate('upload')));
    }
  }, {
    key: "getSearchHTML",
    value: function getSearchHTML() {
      return _react.default.createElement("div", {
        id: "madxartwork-icons-manager__search"
      }, _react.default.createElement("input", {
...
var h=__webpack_require__(237),n="function"===typeof Symbol&&Symbol.for,p=n?Symbol.for("react.element"):60103,q=n?Symbol.for("react.portal"):60106,r=n?Symbol.for("react.fragment"):60107,t=n?Symbol.for("react.strict_mode"):60108,u=n?Symbol.for("react.profiler"):60114,v=n?Symbol.for("react.provider"):60109,w=n?Symbol.for("react.context"):60110,x=n?Symbol.for("react.forward_ref"):60112,y=n?Symbol.for("react.suspense"):60113,aa=n?Symbol.for("react.suspense_list"):60120,ba=n?Symbol.for("react.memo"):
...
var _Object$defineProperties = __webpack_require__(296);

var _Object$getOwnPropertyDescriptors = __webpack_require__(299);
...
*/
var aa=__webpack_require__(180),m=__webpack_require__(237),q=__webpack_require__(308);function t(a){for(var b=a.message,c="https://reactjs.org/docs/error-decoder.html?invariant="+b,d=1;d<arguments.length;d++)c+="&args[]="+encodeURIComponent(arguments[d]);a.message="Minified React error #"+b+"; visit "+c+" for the full message or use the non-minified dev environment for full errors and additional helpful warnings. ";return a}if(!aa)throw t(Error(227));var ba=null,ca={};
...
yc);F[b]=new D(b,1,!1,a,null,!1)});"xlink:actuate xlink:arcrole xlink:role xlink:show xlink:title xlink:type".split(" ").forEach(function(a){var b=a.replace(xc,yc);F[b]=new D(b,1,!1,a,"https://www.w3.org/1999/xlink",!1)});["xml:base","xml:lang","xml:space"].forEach(function(a){var b=a.replace(xc,yc);F[b]=new D(b,1,!1,a,"https://www.w3.org/XML/1998/namespace",!1)});["tabIndex","crossOrigin"].forEach(function(a){F[a]=new D(a,1,!1,a.toLowerCase(),null,!1)});
F.xlinkHref=new D("xlinkHref",1,!1,"xlink:href","https://www.w3.org/1999/xlink",!0);["src","href","action","formAction"].forEach(function(a){F[a]=new D(a,1,!1,a.toLowerCase(),null,!0)});
...
function pe(a,b){var c=Ac(b.value),d=Ac(b.defaultValue);null!=c&&(c=""+c,c!==a.value&&(a.value=c),null==b.defaultValue&&a.defaultValue!==c&&(a.defaultValue=c));null!=d&&(a.defaultValue=""+d)}function qe(a){var b=a.textContent;b===a._wrapperState.initialValue&&(a.value=b)}var re={html:"https://www.w3.org/1999/xhtml",mathml:"https://www.w3.org/1998/Math/MathML",svg:"https://www.w3.org/2000/svg"};
function se(a){switch(a){case "svg":return"https://www.w3.org/2000/svg";case "math":return"https://www.w3.org/1998/Math/MathML";default:return"https://www.w3.org/1999/xhtml"}}function te(a,b){return null==a||"https://www.w3.org/1999/xhtml"===a?se(b):"https://www.w3.org/2000/svg"===a&&"foreignObject"===b?"https://www.w3.org/1999/xhtml":a}
...
var _toConsumableArray2 = _interopRequireDefault(__webpack_require__(311));
...
var _iconList = _interopRequireDefault(__webpack_require__(324));

var _icon = _interopRequireDefault(__webpack_require__(325));
  ...
  function Tab() {
    ...
    (0, _defineProperty2.default)((0, _assertThisInitialized2.default)(_this), "componentDidMount", function () {
      if (_this.props.selected && _this.props.selected.value) {
        setTimeout(function () {
          var element = document.querySelector('.madxartwork-selected');
    ...
    (0, _defineProperty2.default)((0, _assertThisInitialized2.default)(_this), "getLibrary", function (libraryName) {
      var icons = madxartwork.config.icons.libraries.filter(function (library) {
    ...
    (0, _defineProperty2.default)((0, _assertThisInitialized2.default)(_this), "render", function () {
      ...
      try {
        for (var _iterator = (0, _getIterator2.default)(icons.entries()), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
          ...
          if (icon.props.containerClass.includes('madxartwork-selected')) {
            selectedIndex = index;
            break;
          }
        }
  ...
  (0, _createClass2.default)(Tab, [{
    ...
    value: function getIconsOfType(type, icons) {
      ...
      return (0, _entries.default)(icons).map(function (icon) {
        ...
        var containerClass = 'madxartwork-icons-manager__tab__item';

        if (selected.value === className) {
          containerClass += ' madxartwork-selected';
        }
...
var _Array$from = __webpack_require__(314);

var _isIterable = __webpack_require__(320);
...
function (_Component) {
  ...
  function LazyIconList() {
    ...
    (0, _defineProperty2.default)((0, _assertThisInitialized2.default)(_this), "selectors", {
      item: '.madxartwork-icons-manager__tab__item',
      wrapper: 'madxartwork-icons-manager__tab__wrapper'
    });
    ...
    (0, _defineProperty2.default)((0, _assertThisInitialized2.default)(_this), "render", function () {
      ...
      return _react.default.createElement(_react.Fragment, null, _react.default.createElement("div", {
        className: 'madxartwork-icons-manager__tab__content__offset',
        style: offsetStyle
      }), _react.default.createElement("div", {
        id: 'madxartwork-icons-manager__tab__content'
      }, itemsInView), _react.default.createElement("div", {
        className: 'madxartwork-icons-manager__tab__content__tail',
        style: tailStyle
      }));
    });
    return _this;
  ...
  function Icon() {
    ...
    (0, _defineProperty2.default)((0, _assertThisInitialized2.default)(_this), "render", function () {
      return _react.default.createElement("div", {
        ...
        className: "madxartwork-icons-manager__tab__item__content"
      }, _react.default.createElement("i", {
        className: 'madxartwork-icons-manager__tab__item__icon ' + _this.props.className
      }), _react.default.createElement("div", {
        className: 'madxartwork-icons-manager__tab__item__name',
...
function (_Component) {
  ...
  function IconsGoPro() {
    ...
    (0, _defineProperty2.default)((0, _assertThisInitialized2.default)(_this), "render", function () {
      return _react.default.createElement("div", {
        id: "madxartwork-icons-manager__promotion"
      }, _react.default.createElement("i", {
        id: "madxartwork-icons-manager__promotion__icon",
        className: "eicon-nerd"
      }), _react.default.createElement("div", {
        id: "madxartwork-icons-manager__promotion__text"
      }, madxartwork.translate('icons_promotion')), _react.default.createElement("a", {
        href: madxartwork.config.icons.goProURL,
        id: "madxartwork-icons-manager__promotion__link",
        target: "_blank",
        rel: "noopener noreferrer"
      }, madxartwork.translate('go_pro')));
    });
    return _this;
  }
...
function () {
  function _default() {
    ...
    (0, _defineProperty2.default)(this, "initIconType", function (libraryConfig, callback) {
      ...
      var store = madxartwork.iconManager.store;
      ...
      if (libraryConfig.enqueue) {
        libraryConfig.enqueue.forEach(function (assetURL) {
          madxartwork.helpers.enqueueEditorStylesheet(assetURL);
        });
      }

      if (libraryConfig.url) {
        madxartwork.helpers.enqueueEditorStylesheet(libraryConfig.url);
    ...
    value: function normalizeIconList(library) {
      ...
      jQuery.each(library.icons, function (index, icon) {
        ...
        icons[name] = {
          ...
          name: madxartworkCommon.helpers.upperCaseWords(name).trim(':').split('-').join(' '),
          filter: name.trim(':'),
      ...
      if ((0, _keys.default)(icons).length) {
        ...
        madxartwork.iconManager.store.save(library);
...
function () {
  ...
  (0, _createClass2.default)(Store, [{
    ...
    value: function save(library) {
      madxartworkCommon.storage.set(Store.getKey(library), library);
    }
  }, {
    key: "getIcons",
    ...
    value: function get(key) {
      return madxartworkCommon.storage.get(key);
    }
  }, {
    key: "isValid",
    ...
    value: function getKey(library) {
      ...
      return "madxartwork_".concat(name, "_icons");
    }
  }]);
  return Store;
}();
...
function (_madxartworkModules$com) {
  ...
  (0, _createClass2.default)(_default, [{
    ...
    value: function dependency() {
      return !!madxartwork.getCurrentElement();
    }
  }, {
    key: "defaultCommands",
    value: function defaultCommands() {
      return {
        copy: function copy() {
          return madxartwork.getCurrentElement().copy();
        },
        duplicate: function duplicate() {
          return madxartwork.getCurrentElement().duplicate();
        },
        delete: function _delete() {
          return madxartwork.getCurrentElement().removeElement();
        },
        paste: function paste() {
          return madxartwork.getCurrentElement().paste();
        },
        pasteStyle: function pasteStyle() {
          return madxartwork.getCurrentElement().pasteStyle();
        }
      };
    }
  }, {
    ...
    value: function defaultShortcuts() {
      return {
        ...
        paste: {
          ...
          exclude: ['input'],
          dependency: function dependency() {
            return madxartwork.getCurrentElement().isPasteEnabled();
          }
        },
        pasteStyle: {
          keys: 'ctrl+shift+v',
          exclude: ['input'],
          dependency: function dependency() {
            return madxartwork.getCurrentElement().pasteStyle && madxartworkCommon.storage.get('transfer');
          }
        }
      };
    }
...
helpers = {
  ...
  enqueuePreviewStylesheet: function enqueuePreviewStylesheet(url) {
    this.enqueueCSS(url, madxartwork.$previewContents);
  },
  enqueueEditorStylesheet: function enqueueEditorStylesheet(url) {
    this.enqueueCSS(url, madxartworkCommon.elements.$document);
  },
  ...
  enqueueStylesheet: function enqueueStylesheet(url) {
    madxartworkCommon.helpers.deprecatedMethod('madxartwork.helpers.enqueueStylesheet()', '2.6.0', 'madxartwork.helpers.enqueuePreviewStylesheet()');
  ...
  getInlineSvg: function getInlineSvg(value, view) {
    ...
    this.fetchInlineSvg(value.url, function (data) {
      if (data) {
        ...
        madxartwork.channels.editor.trigger('svg:insertion', data, value.id);
      }
    });
  },
  enqueueIconFonts: function enqueueIconFonts(iconType) {
    ...
    if (-1 !== this._enqueuedIconFonts.indexOf(iconType) || !!madxartworkConfig['icons_update_needed']) {
      return;
    }
    ...
    madxartwork.channels.editor.trigger('fontIcon:insertion', iconType, iconSetting);
  },
  getIconLibrarySettings: function getIconLibrarySettings(iconType) {
    var iconSetting = madxartwork.config.icons.libraries.filter(function (library) {
  ...
  renderIcon: function renderIcon(view, icon) {
    ...
    madxartwork.channels.editor.trigger('Icon:insertion', iconType, iconValue, attributes, tag, view);
  ...
  fetchFa4ToFa5Mapping: function fetchFa4ToFa5Mapping() {
    ...
    var mapping = madxartworkCommon.storage.get(storageKey);

    if (!mapping) {
      jQuery.getJSON(madxartworkConfig.fa4_to_fa5_mapping_url, function (data) {
        mapping = data;
        madxartworkCommon.storage.set(storageKey, data);
      });
    }
  ...
  enqueueFont: function enqueueFont(font) {
    ...
    var fontType = madxartwork.config.controls.font.options[font],
        subsets = {
      ru_RU: 'cyrillic',
      uk: 'cyrillic',
      bg_BG: 'cyrillic',
    ...
    switch (fontType) {
      case 'googlefonts':
        ...
        if (subsets[madxartwork.config.locale]) {
          fontUrl += '&subset=' + subsets[madxartwork.config.locale];
        }
    ...
    madxartwork.channels.editor.trigger('font:insertion', fontType, font);
  },
  ...
  getSocialNetworkNameFromIcon: function getSocialNetworkNameFromIcon(iconsControl, fallbackControl) {
    ...
    social = madxartwork.hooks.applyFilters('madxartwork/social_icons/network_name', social, iconsControl, fallbackControl, toUpperCase, withIcon);
  ...
  maybeDisableWidget: function maybeDisableWidget() {
    if (!madxartworkConfig['icons_update_needed']) {
      return false;
    }

    var elementView = madxartwork.channels.panelElements.request('element:selected'),
        widgetType = elementView.model.get('widgetType'),
        widgetData = madxartwork.config.widgets[widgetType],
    ...
    if (widgetData) {
      ...
      if (hasIconsControl) {
        var onConfirm = function onConfirm() {
          window.location.href = madxartworkConfig.tools_page_link + '&redirect_to=' + encodeURIComponent(document.location.href) + '#tab-fontawesome4_migration';
        };

        madxartwork.helpers.getSimpleDialog('madxartwork-enable-fa5-dialog', madxartwork.translate('enable_fa5'), madxartwork.translate('dialog_confirm_enable_fa5'), madxartwork.translate('update'), onConfirm).show();
        return true;
      }
    }
  ...
  */
  ...
  isActiveControl: function isActiveControl(controlModel, values) {
    ...
    if (conditions && !madxartwork.conditions.check(conditions, values)) {
      return false;
    }
  ...
  cloneObject: function cloneObject(object) {
    madxartworkCommon.helpers.deprecatedMethod('madxartwork.helpers.cloneObject', '2.3.0', 'madxartworkCommon.helpers.cloneObject');
    return madxartworkCommon.helpers.cloneObject(object);
  },
  firstLetterUppercase: function firstLetterUppercase(string) {
    madxartworkCommon.helpers.deprecatedMethod('madxartwork.helpers.upperCaseWords', '2.3.0', 'madxartworkCommon.helpers.upperCaseWords');
    return madxartworkCommon.helpers.upperCaseWords(string);
  },
  ...
  wpColorPicker: function wpColorPicker($element, options) {
    var self = this,
        colorPickerScheme = madxartwork.schemes.getScheme('color-picker'),
  ...
  scrollToView: function scrollToView($element, timeout, $parent) {
    ...
    var $scrolled = $parent;
    var $madxartworkFrontendWindow = madxartworkFrontend.elements.$window;

    if (!$parent) {
      $parent = $madxartworkFrontendWindow;
      $scrolled = madxartwork.$previewContents.find('html, body');
    }

    setTimeout(function () {
      var parentHeight = $parent.height(),
          parentScrollTop = $parent.scrollTop(),
          elementTop = $parent === $madxartworkFrontendWindow ? $element.offset().top : $element[0].offsetTop,
  ...
  elementSizeToUnit: function elementSizeToUnit($element, size, unit) {
    var window = madxartworkFrontend.elements.window;
  ...
  compareVersions: function compareVersions(versionA, versionB, operator) {
    ...
    for (var i = 0; i < longestVersionParts; i++) {
      ...
      if (valueA !== valueB) {
        return madxartwork.conditions.compare(valueA, valueB, operator);
      }
    }
  }
};
...
var dP = __webpack_require__(37).f;
var $trim = __webpack_require__(334).trim;
var NUMBER = 'Number';
...
module.exports = {
  set: Object.setPrototypeOf || ('__proto__' in {} ? // eslint-disable-line
    function (test, buggy, set) {
      try {
        set = __webpack_require__(59)(Function.call, __webpack_require__(241).f(Object.prototype, '__proto__').set, 2);
        set(test, []);
        buggy = !(test instanceof Array);
      } catch (e) { buggy = true; }
      return function setPrototypeOf(O, proto) {
...
var defined = __webpack_require__(32);
var fails = __webpack_require__(24);
var spaces = __webpack_require__(335);
var space = '[' + spaces + ']';
var non = '\u200b\u0085';
var ltrim = RegExp('^' + space + space + '*');
var rtrim = RegExp(space + space + '*$');
...
ImagesManager = function ImagesManager() {
  ...
  self.updateOnReceiveImage = function () {
    var elementView = madxartwork.getPanelView().getCurrentPageView().getOption('editedElementView');
    elementView.$el.addClass('madxartwork-loading'); // Add per cid for multiple images in a single view.

    viewsToUpdate[elementView.cid] = elementView;
    madxartwork.channels.editor.once('imagesManager:detailsReceived', function () {
      if (!_.isEmpty(viewsToUpdate)) {
        _(viewsToUpdate).each(function (view) {
          view.render();
          view.$el.removeClass('madxartwork-loading');
        });
      }
  ...
  self.getRemoteItems = function () {
    ...
    madxartworkCommon.ajax.send('get_images_details', {
      data: {
      ...
      success: function success(data) {
        ...
        registeredItems = [];
        madxartwork.channels.editor.trigger('imagesManager:detailsReceived', data);
      }
    });
  };
...
var Debug = function Debug() {
  ...
  var initSettings = function initSettings() {
    settings = {
      debounceDelay: 500,
      urlsToWatch: ['madxartwork/assets']
    };
  };
  ...
  this.sendErrors = function () {
    ...
    jQuery.ajax({
      url: madxartworkCommon.config.ajax.url,
      method: 'POST',
      data: {
        action: 'madxartwork_js_log',
        _nonce: madxartworkCommon.ajax.getSettings('nonce'),
        data: errorStack
      },
      success: function success() {
...
Schemes = function Schemes() {
  var self = this,
      ...
      schemes = {},
      settings = {
    selectorWrapperPrefix: '.madxartwork-widget-'
  },
      elements = {};
  ...
  var initElements = function initElements() {
    elements.$style = jQuery('<style>', {
      id: 'madxartwork-style-scheme'
    });
    elements.$previewHead = madxartwork.$previewContents.find('head');
  };

  var initSchemes = function initSchemes() {
    schemes = madxartworkCommon.helpers.cloneObject(madxartwork.config.schemes.items);
  };
  ...
  var fetchAllWidgetsSchemesStyle = function fetchAllWidgetsSchemesStyle() {
    _.each(madxartwork.config.widgets, function (widget) {
  ...
  this.getEnabledSchemesTypes = function () {
    return madxartwork.config.schemes.enabled_schemes;
  };
  ...
  this.getSchemeValue = function (schemeType, value, key) {
    ...
    if (key && _.isObject(schemeValue)) {
      var clonedSchemeValue = madxartworkCommon.helpers.cloneObject(schemeValue);
  ...
  this.resetSchemes = function (schemeName) {
    schemes[schemeName] = madxartworkCommon.helpers.cloneObject(madxartwork.config.schemes.items[schemeName]);
  };

  this.saveScheme = function (schemeName) {
    madxartwork.config.schemes.items[schemeName].items = madxartworkCommon.helpers.cloneObject(schemes[schemeName].items);
    var itemsToSave = {};
    ...
    NProgress.start();
    madxartworkCommon.ajax.addRequest('apply_scheme', {
      data: {
...
presetsFactory = {
  ...
  getAbsolutePresetValues: function getAbsolutePresetValues(preset) {
    var clonedPreset = madxartworkCommon.helpers.cloneObject(preset),
  ...
  getPresets: function getPresets(columnsCount, presetIndex) {
    var presets = madxartworkCommon.helpers.cloneObject(madxartwork.config.elements.section.presets);
...
var _component = _interopRequireDefault(__webpack_require__(342));

var TemplateLibraryCollection = __webpack_require__(355),
...
TemplateLibraryManager = function TemplateLibraryManager() {
  ...
  var registerDefaultTemplateTypes = function registerDefaultTemplateTypes() {
    var data = {
      saveDialog: {
        description: madxartwork.translate('save_your_template_description')
      },
      ajaxParams: {
    ...
    _.each(['page', 'section', madxartwork.config.document.type], function (type) {
      var safeData = jQuery.extend(true, {}, data, {
        saveDialog: {
          title: madxartwork.translate('save_your_template', [madxartwork.translate(type)])
        }
      });
  ...
  this.init = function () {
    ...
    madxartwork.addBackgroundClickListener('libraryToggleMore', {
      element: '.madxartwork-template-library-template-more'
    });
  };
  ...
  this.deleteTemplate = function (templateModel, options) {
    ...
    dialog.onConfirm = function () {
      ...
      madxartworkCommon.ajax.addRequest('delete_template', {
        data: {
  ...
  this.importTemplate = function (templateModel, options) {
    ...
    self.requestTemplateContent(templateModel.get('source'), templateModel.get('template_id'), {
      ...
      success: function success(data) {
        ...
        madxartwork.channels.data.trigger('template:before:insert', templateModel);
        madxartwork.getPreviewView().addChildModel(data.content, importOptions);
        madxartwork.channels.data.trigger('template:after:insert', templateModel);

        if (options.withPageSettings) {
          madxartwork.settings.page.model.setExternalChange(data.page_settings);
        }
      },
  ...
  this.saveTemplate = function (type, data) {
    ...
    madxartworkCommon.ajax.addRequest('save_template', ajaxParams);
  };
  ...
  this.requestTemplateContent = function (source, id, ajaxOptions) {
    ...
    return madxartworkCommon.ajax.addRequest('get_template_data', options);
  };
  ...
  this.markAsFavorite = function (templateModel, favorite) {
    ...
    return madxartworkCommon.ajax.addRequest('mark_template_as_favorite', options);
  };

  this.getDeleteDialog = function () {
    if (!deleteDialog) {
      deleteDialog = madxartworkCommon.dialogsManager.createWidget('confirm', {
        id: 'madxartwork-template-library-delete-dialog',
        headerMessage: madxartwork.translate('delete_template'),
        message: madxartwork.translate('delete_template_confirm'),
        strings: {
          confirm: madxartwork.translate('delete')
        }
      });
  ...
  this.getErrorDialog = function () {
    if (!errorDialog) {
      errorDialog = madxartworkCommon.dialogsManager.createWidget('alert', {
        id: 'madxartwork-template-library-error-dialog',
        headerMessage: madxartwork.translate('an_error_occurred')
      });
    }
  ...
  this.requestLibraryData = function (options) {
    ...
    madxartworkCommon.ajax.addRequest('get_library_data', ajaxOptions);
  };

  this.getFilter = function (name) {
    return madxartwork.channels.templates.request('filter:' + name);
  };

  this.setFilter = function (name, value, silent) {
    madxartwork.channels.templates.reply('filter:' + name, value);

    if (!silent) {
      madxartwork.channels.templates.trigger('filter:change');
    }
  };
  ...
  this.setScreen = function (args) {
    madxartwork.channels.templates.stopReplying();
  ...
  this.showErrorDialog = function (errorMessage) {
    ...
    self.getErrorDialog().setMessage(madxartwork.translate('templates_request_error') + '<div id="madxartwork-template-library-error-info">' + errorMessage + '</div>').show();
  };
};
...
var TemplateLibraryLayoutView = __webpack_require__(343);
...
function (_madxartworkModules$com) {
  ...
  (0, _createClass2.default)(_default, [{
    ...
    value: function __construct(args) {
      ...
      this.docLibraryConfig = madxartwork.config.document.remoteLibrary;
    ...
    value: function defaultTabs() {
      return {
        'templates/blocks': {
          title: madxartwork.translate('blocks'),
          filter: {
            source: 'remote',
            type: 'block',
        ...
        'templates/pages': {
          title: madxartwork.translate('pages'),
          filter: {
            source: 'remote',
            type: 'page'
          }
        ...
        'templates/my-templates': {
          title: madxartwork.translate('my_templates'),
          filter: {
            source: 'local'
          }
        }
    ...
    value: function getTabsWrapperSelector() {
      return '#madxartwork-template-library-header-menu';
    }
  }, {
    key: "renderTab",
...
var TemplateLibraryHeaderActionsView = __webpack_require__(344),
    TemplateLibraryHeaderMenuView = __webpack_require__(345),
    TemplateLibraryHeaderPreviewView = __webpack_require__(346),
    TemplateLibraryHeaderBackView = __webpack_require__(347),
    TemplateLibraryCollectionView = __webpack_require__(348),
    TemplateLibrarySaveTemplateView = __webpack_require__(352),
    TemplateLibraryImportView = __webpack_require__(353),
    TemplateLibraryPreviewView = __webpack_require__(354);

module.exports = madxartworkModules.common.views.modal.Layout.extend({
  getModalOptions: function getModalOptions() {
    return {
      id: 'madxartwork-template-library-modal'
    };
  },
  getLogoOptions: function getLogoOptions() {
    return {
      title: madxartwork.translate('library'),
  ...
  getTemplateActionButton: function getTemplateActionButton(templateData) {
    var viewId = '#tmpl-madxartwork-template-library-' + (templateData.isPro ? 'get-pro-button' : 'insert-button');
    viewId = madxartwork.hooks.applyFilters('madxartwork/editor/template-library/template/action-button', viewId, templateData);
...
module.exports = Marionette.ItemView.extend({
  template: '#tmpl-madxartwork-template-library-header-actions',
  id: 'madxartwork-template-library-header-actions',
  ui: {
    import: '#madxartwork-template-library-header-import i',
    sync: '#madxartwork-template-library-header-sync i',
    save: '#madxartwork-template-library-header-save i'
  },
  events: {
  ...
  onSyncClick: function onSyncClick() {
    ...
    madxartwork.templates.requestLibraryData({
      onUpdate: function onUpdate() {
...
module.exports = Marionette.ItemView.extend({
  template: '#tmpl-madxartwork-template-library-header-menu',
  id: 'madxartwork-template-library-header-menu',
  templateHelpers: function templateHelpers() {
    return {
...
module.exports = Marionette.ItemView.extend({
  template: '#tmpl-madxartwork-template-library-header-preview',
  id: 'madxartwork-template-library-header-preview',
  behaviors: {
    insertTemplate: {
      behaviorClass: TemplateLibraryInsertTemplateBehavior
    }
...
module.exports = Marionette.ItemView.extend({
  template: '#tmpl-madxartwork-template-library-header-back',
  id: 'madxartwork-template-library-header-preview-back',
  events: {
    click: 'onClick'
  },
  onClick: function onClick() {
...
var TemplateLibraryTemplateLocalView = __webpack_require__(349),
    TemplateLibraryTemplateRemoteView = __webpack_require__(350),
    TemplateLibraryCollectionView;

TemplateLibraryCollectionView = Marionette.CompositeView.extend({
  template: '#tmpl-madxartwork-template-library-templates',
  id: 'madxartwork-template-library-templates',
  childViewContainer: '#madxartwork-template-library-templates-container',
  reorderOnSort: true,
  emptyView: function emptyView() {
    var EmptyView = __webpack_require__(351);

    return new EmptyView();
  },
  ui: {
    textFilter: '#madxartwork-template-library-filter-text',
    selectFilter: '.madxartwork-template-library-filter-select',
    myFavoritesFilter: '#madxartwork-template-library-filter-my-favorites',
    orderInputs: '.madxartwork-template-library-order-input',
    orderLabels: 'label.madxartwork-template-library-order-label'
  },
  events: {
  ...
  initialize: function initialize() {
    this.listenTo(madxartwork.channels.templates, 'filter:change', this._renderChildren);
  },
  filter: function filter(childModel) {
    var filterTerms = madxartwork.templates.getFilterTerms(),
        passingFilter = true;
    jQuery.each(filterTerms, function (filterTermName) {
      var filterValue = madxartwork.templates.getFilter(filterTermName);
  ...
  addSourceData: function addSourceData() {
    ...
    this.$el.attr('data-template-source', isEmpty ? 'empty' : madxartwork.templates.getFilter('source'));
  },
  setFiltersUI: function setFiltersUI() {
    var $filters = this.$(this.ui.selectFilter);
    $filters.select2({
      placeholder: madxartwork.translate('category'),
      allowClear: true,
      width: 150
    });
  },
  ...
  toggleFilterClass: function toggleFilterClass() {
    this.$el.toggleClass('madxartwork-templates-filter-active', !!(madxartwork.templates.getFilter('text') || madxartwork.templates.getFilter('favorite')));
  },
  onRender: function onRender() {
    if ('remote' === madxartwork.templates.getFilter('source') && 'page' !== madxartwork.templates.getFilter('type')) {
      this.setFiltersUI();
    }
  },
  onRenderCollection: function onRenderCollection() {
  ...
  onTextFilterInput: function onTextFilterInput() {
    madxartwork.templates.setFilter('text', this.ui.textFilter.val());
  },
  onSelectFilterChange: function onSelectFilterChange(event) {
    var $select = jQuery(event.currentTarget),
        filterName = $select.data('madxartwork-filter');
    madxartwork.templates.setFilter(filterName, $select.val());
  },
  onMyFavoritesFilterChange: function onMyFavoritesFilterChange() {
    madxartwork.templates.setFilter('favorite', this.ui.myFavoritesFilter[0].checked);
  },
  onOrderLabelsClick: function onOrderLabelsClick(event) {
    ...
    $clickedInput.toggleClass('madxartwork-template-library-order-reverse', toggle);
    this.order($clickedInput.val(), $clickedInput.hasClass('madxartwork-template-library-order-reverse'));
  }
});
...
TemplateLibraryTemplateLocalView = TemplateLibraryTemplateView.extend({
  template: '#tmpl-madxartwork-template-library-template-local',
  ui: function ui() {
    return _.extend(TemplateLibraryTemplateView.prototype.ui.apply(this, arguments), {
      deleteButton: '.madxartwork-template-library-template-delete',
      morePopup: '.madxartwork-template-library-template-more',
      toggleMore: '.madxartwork-template-library-template-more-toggle',
      toggleMoreIcon: '.madxartwork-template-library-template-more-toggle i'
    });
  },
  events: function events() {
...
TemplateLibraryTemplateRemoteView = TemplateLibraryTemplateView.extend({
  template: '#tmpl-madxartwork-template-library-template-remote',
  ui: function ui() {
    return jQuery.extend(TemplateLibraryTemplateView.prototype.ui.apply(this, arguments), {
      favoriteCheckbox: '.madxartwork-template-library-template-favorite-input'
    });
  },
  events: function events() {
  ...
  onFavoriteCheckboxChange: function onFavoriteCheckboxChange() {
    ...
    madxartwork.templates.markAsFavorite(this.model, isFavorite);

    if (!isFavorite && madxartwork.templates.getFilter('favorite')) {
      madxartwork.channels.templates.trigger('filter:change');
    }
  }
});
...
TemplateLibraryTemplatesEmptyView = Marionette.ItemView.extend({
  id: 'madxartwork-template-library-templates-empty',
  template: '#tmpl-madxartwork-template-library-templates-empty',
  ui: {
    title: '.madxartwork-template-library-blank-title',
    message: '.madxartwork-template-library-blank-message'
  },
  modesStrings: {
    empty: {
      title: madxartwork.translate('templates_empty_title'),
      message: madxartwork.translate('templates_empty_message')
    },
    noResults: {
      title: madxartwork.translate('templates_no_results_title'),
      message: madxartwork.translate('templates_no_results_message')
    },
    noFavorites: {
      title: madxartwork.translate('templates_no_favorites_title'),
      message: madxartwork.translate('templates_no_favorites_message')
    }
  },
  getCurrentMode: function getCurrentMode() {
    if (madxartwork.templates.getFilter('text')) {
      return 'noResults';
    }

    if (madxartwork.templates.getFilter('favorite')) {
      return 'noFavorites';
    }
...
TemplateLibrarySaveTemplateView = Marionette.ItemView.extend({
  id: 'madxartwork-template-library-save-template',
  template: '#tmpl-madxartwork-template-library-save-template',
  ui: {
    form: '#madxartwork-template-library-save-template-form',
    submitButton: '#madxartwork-template-library-save-template-submit'
  },
  events: {
  ...
  getSaveType: function getSaveType() {
    ...
    } else if (madxartwork.config.document.library && madxartwork.config.document.library.save_as_same_type) {
      type = madxartwork.config.document.type;
    } else {
      type = 'page';
    }
  ...
  templateHelpers: function templateHelpers() {
    var saveType = this.getSaveType(),
        templateType = madxartwork.templates.getTemplateTypes(saveType);
    return templateType.saveDialog;
  },
  onFormSubmit: function onFormSubmit(event) {
    event.preventDefault();
    var formData = this.ui.form.madxartworkSerializeObject(),
        saveType = this.getSaveType(),
        JSONParams = {
      remove: ['default']
    };
    formData.content = this.model ? [this.model.toJSON(JSONParams)] : madxartwork.elements.toJSON(JSONParams);
    this.ui.submitButton.addClass('madxartwork-button-state');
    madxartwork.templates.saveTemplate(saveType, formData);
  }
});
...
TemplateLibraryImportView = Marionette.ItemView.extend({
  template: '#tmpl-madxartwork-template-library-import',
  id: 'madxartwork-template-library-import',
  ui: {
    uploadForm: '#madxartwork-template-library-import-form',
    fileInput: '#madxartwork-template-library-import-form-input'
  },
  events: {
  ...
  importTemplate: function importTemplate(fileName, fileData) {
    var layout = madxartwork.templates.layout;
    var options = {
      data: {
        fileName: fileName,
        fileData: fileData
      ...
      success: function success(successData) {
        madxartwork.templates.getTemplatesCollection().add(successData);
        $e.route('library/templates/my-templates');
      },
      error: function error(errorData) {
        madxartwork.templates.showErrorDialog(errorData);
    ...
    madxartworkCommon.ajax.addRequest('import_template', options);
  ...
  onFormDragEnter: function onFormDragEnter() {
    this.ui.uploadForm.addClass('madxartwork-drag-over');
  },
  onFormDragLeave: function onFormDragLeave(event) {
    ...
    this.ui.uploadForm.removeClass('madxartwork-drag-over');
  },
...
TemplateLibraryPreviewView = Marionette.ItemView.extend({
  template: '#tmpl-madxartwork-template-library-preview',
  id: 'madxartwork-template-library-preview',
  ui: {
    iframe: '> iframe'
  },
  onRender: function onRender() {
...
var TemplateLibraryTemplateModel = __webpack_require__(356),
...
var _component = _interopRequireDefault(__webpack_require__(359));

var HistoryPageView = __webpack_require__(360),
    Manager;
...
Manager = function Manager() {
  ...
  var addPanelPage = function addPanelPage() {
    madxartwork.getPanelView().addPage('historyPage', {
      view: HistoryPageView,
      title: madxartwork.translate('history')
    });
  };

  var init = function init() {
    madxartwork.on('preview:loaded', addPanelPage);
    $e.components.register(new _component.default({
      manager: self
    }));
    self.history = __webpack_require__(368);
    self.revisions = __webpack_require__(378);
    self.revisions.init();
  };

  jQuery(window).on('madxartwork:init', init);
};
...
function (_madxartworkModules$com) {
  ...
  (0, _createClass2.default)(_default, [{
    ...
    value: function defaultTabs() {
      return {
        actions: {
          title: madxartwork.translate('actions')
        },
        revisions: {
          title: madxartwork.translate('revisions')
        }
      };
    }
  }, {
    ...
    key: "getTabsWrapperSelector",
    value: function getTabsWrapperSelector() {
      return '#madxartwork-panel-elements-navigation';
    }
  }, {
    key: "renderTab",
    value: function renderTab(tab) {
      madxartwork.getPanelView().setPage('historyPage').showView(tab);
    }
  }]);
  return _default;
}(madxartworkModules.common.Component);
...
var _loading = _interopRequireDefault(__webpack_require__(361));

var _panelTab = _interopRequireDefault(__webpack_require__(362));

var _empty = _interopRequireDefault(__webpack_require__(364));

var TabHistoryView = __webpack_require__(365);

module.exports = Marionette.LayoutView.extend({
  template: '#tmpl-madxartwork-panel-history-page',
  regions: {
    content: '#madxartwork-panel-history-content'
  },
  ui: {
    tabs: '.madxartwork-panel-navigation-tab'
  },
  regionViews: {},
  currentTab: null,
  initialize: function initialize() {
  ...
  initRegionViews: function initRegionViews() {
    var historyItems = madxartwork.history.history.getItems();
    this.regionViews = {
      actions: {
        view: function view() {
      ...
      revisions: {
        view: function view() {
          var revisionsItems = madxartwork.history.revisions.getItems();
...
function (_Marionette$ItemView) {
  ...
  (0, _createClass2.default)(_default, [{
    key: "getTemplate",
    value: function getTemplate() {
      return '#tmpl-madxartwork-panel-revisions-loading';
    }
  }, {
    key: "id",
    value: function id() {
      return 'madxartwork-panel-revisions-loading';
    }
  }, {
    key: "onRender",
    value: function onRender() {
      madxartwork.history.revisions.requestRevisions(function () {
        setTimeout(function () {
...
module.exports = Marionette.CompositeView.extend({
  id: 'madxartwork-panel-revisions',
  template: '#tmpl-madxartwork-panel-revisions',
  childView: __webpack_require__(363),
  childViewContainer: '#madxartwork-revisions-list',
  ui: {
    discard: '.madxartwork-panel-scheme-discard .madxartwork-button',
    apply: '.madxartwork-panel-scheme-save .madxartwork-button'
  },
  events: {
  ...
  initialize: function initialize() {
    this.collection = madxartwork.history.revisions.getItems();
    this.listenTo(madxartwork.channels.editor, 'saved', this.onEditorSaved);
    this.currentPreviewId = madxartwork.config.current_revision_id;
  },
  getRevisionViewData: function getRevisionViewData(revisionView) {
    var self = this;
    madxartwork.history.revisions.getRevisionDataAsync(revisionView.model.get('id'), {
      success: function success(data) {
        madxartwork.history.revisions.setEditorData(data.elements);
        madxartwork.settings.page.model.set(data.settings);
        self.setRevisionsButtonsActive(true);
        revisionView.$el.removeClass('madxartwork-revision-item-loading');
  ...
  deleteRevision: function deleteRevision(revisionView) {
    var self = this;
    revisionView.$el.addClass('madxartwork-revision-item-loading');
    madxartwork.history.revisions.deleteRevision(revisionView.model, {
      success: function success() {
  ...
  enterReviewMode: function enterReviewMode() {
    madxartwork.changeEditMode('review');
  },
  exitReviewMode: function exitReviewMode() {
    madxartwork.changeEditMode('edit');
  },
  ...
  onApplyClick: function onApplyClick() {
    ...
    madxartwork.history.history.getItems().reset();
  },
  onDiscardClick: function onDiscardClick() {
    madxartwork.history.revisions.setEditorData(madxartwork.config.data);
    madxartwork.saver.setFlagEditorChange(this.isRevisionApplied);
    ...
    if (this.currentPreviewItem) {
      this.currentPreviewItem.$el.removeClass('madxartwork-revision-current-preview');
    }
  },
  onDestroy: function onDestroy() {
    if (this.currentPreviewId && this.currentPreviewId !== madxartwork.config.current_revision_id) {
      this.onDiscardClick();
    }
  },
  onRenderCollection: function onRenderCollection() {
    ...
    if (currentPreviewModel) {
      ...
      this.currentPreviewItem.$el.addClass('madxartwork-revision-current-preview');
    }
  },
  onChildviewDetailsAreaClick: function onChildviewDetailsAreaClick(childView) {
    ...
    if (self.currentPreviewItem) {
      self.currentPreviewItem.$el.removeClass('madxartwork-revision-current-preview madxartwork-revision-item-loading');
    }

    childView.$el.addClass('madxartwork-revision-current-preview madxartwork-revision-item-loading');

    if (madxartwork.saver.isEditorChanged() && (null === self.currentPreviewId || madxartwork.config.current_revision_id === self.currentPreviewId)) {
      madxartwork.saver.saveEditor({
        status: 'autosave',
        onSuccess: function onSuccess() {
  ...
  onChildviewDeleteClick: function onChildviewDeleteClick(childView) {
    ...
    var removeDialog = madxartworkCommon.dialogsManager.createWidget('confirm', {
      message: madxartwork.translate('dialog_confirm_delete', [type]),
      headerMessage: madxartwork.translate('delete_element', [type]),
      strings: {
        confirm: madxartwork.translate('delete'),
        cancel: madxartwork.translate('cancel')
      },
...
module.exports = Marionette.ItemView.extend({
  template: '#tmpl-madxartwork-panel-revisions-revision-item',
  className: 'madxartwork-revision-item',
  ui: {
    detailsArea: '.madxartwork-revision-item__details',
    deleteButton: '.madxartwork-revision-item__tools-delete'
  },
  triggers: {
...
module.exports = Marionette.ItemView.extend({
  template: '#tmpl-madxartwork-panel-revisions-no-revisions',
  id: 'madxartwork-panel-revisions-no-revisions',
  className: 'madxartwork-nerd-box'
});
...
var _itemView = _interopRequireDefault(__webpack_require__(366));

var _empty = _interopRequireDefault(__webpack_require__(367));

module.exports = Marionette.CompositeView.extend({
  id: 'madxartwork-panel-history',
  template: '#tmpl-madxartwork-panel-history-tab',
  childView: _itemView.default,
  childViewContainer: '#madxartwork-history-list',
  ...
  updateCurrentItem: function updateCurrentItem() {
    ...
    _.defer(function () {
      ...
      var currentItemClass = 'madxartwork-history-item-current';
  ...
  onRenderEmpty: function onRenderEmpty() {
    this.$el.addClass('madxartwork-empty');
  },
  onChildviewClick: function onChildviewClick(childView, event) {
    ...
    madxartwork.history.history.doItem(itemIndex);
  }
});
...
function (_Marionette$ItemView) {
  ...
  (0, _createClass2.default)(_default, [{
    key: "getTemplate",
    value: function getTemplate() {
      return '#tmpl-madxartwork-panel-history-item';
    }
  }, {
    key: "className",
    value: function className() {
      return 'madxartwork-history-item madxartwork-history-item-' + this.model.get('status');
    }
  }, {
    key: "triggers",
...
function (_Marionette$ItemView) {
  ...
  (0, _createClass2.default)(_default, [{
    key: "getTemplate",
    value: function getTemplate() {
      return '#tmpl-madxartwork-panel-history-no-items';
    }
  }, {
    key: "id",
    value: function id() {
      return 'madxartwork-panel-history-no-items';
    }
  }, {
    key: "onDestroy",
    value: function onDestroy() {
      this._parent.$el.removeClass('madxartwork-empty');
    }
  }]);
  return _default;
}(Marionette.ItemView);
...
var _itemModel = _interopRequireDefault(__webpack_require__(369));

var _component = _interopRequireDefault(__webpack_require__(370));

var ElementHistoryBehavior = __webpack_require__(371),
    CollectionHistoryBehavior = __webpack_require__(372);
...
var Manager = function Manager() {
  var self = this,
    ...
    model: _itemModel.default
      ...
      active = true;
  var translations = {
    add: madxartwork.translate('added'),
    remove: madxartwork.translate('removed'),
    change: madxartwork.translate('edited'),
    move: madxartwork.translate('moved'),
    paste_style: madxartwork.translate('style_pasted'),
    reset_style: madxartwork.translate('style_reset')
  };
  ...
  var updatePanelPageCurrentItem = function updatePanelPageCurrentItem() {
    if ($e.routes.is('panel/history/actions')) {
      madxartwork.getPanelView().getCurrentPageView().getCurrentTab().updateCurrentItem();
    }
  };
  ...
  this.addItem = function (itemData) {
    ...
    if (!items.length) {
      items.add({
        status: 'not_applied',
        title: madxartwork.translate('editing_started'),
        subTitle: '',
        action: '',
  ...
  this.doItem = function (index) {
    ...
    if (viewToScroll && !madxartwork.helpers.isInViewport(viewToScroll.$el[0], madxartwork.$previewContents.find('html')[0])) {
      madxartwork.helpers.scrollToView(viewToScroll.$el);
    }

    if (item.get('editing_started')) {
      if (!editorSaved) {
        madxartwork.saver.setFlagEditorChange(false);
      }
    }
  };
  ...
  this.getModelLabel = function (model) {
    ...
    return madxartwork.getElementData(model).title;
  };
  ...
  this.findView = function (modelID, views) {
    ...
    if (!views) {
      views = madxartwork.getPreviewView().children;
    }
  ...
  this.startInsertTemplate = function (model) {
    madxartwork.history.history.startItem({
      type: 'add',
      title: madxartwork.translate('template'),
...
module.exports = Marionette.Behavior.extend({
  ...
  saveTextHistory: function saveTextHistory(model, changed, control) {
    ...
    var historyItem = {
      type: 'change',
      elementType: 'control',
      title: madxartwork.history.history.getModelLabel(model),
    ...
    madxartwork.history.history.addItem(historyItem);
    delete this.oldValues[control.name];
  },
  saveHistory: function saveHistory(model, options) {
    if (!madxartwork.history.history.getActive()) {
      return;
    }
    ...
    if (1 === changed.length) {
      ...
      if (madxartwork.history.history.isItemStarted()) {
  ...
  restore: function restore(historyItem, isRedo) {
    var history = historyItem.get('history'),
        ...
        view = madxartwork.history.history.findView(modelID);
...
module.exports = Marionette.Behavior.extend({
  ...
  onDeleteAllContent: function onDeleteAllContent(collection, event) {
    ...
    var historyItem = {
      type: 'remove',
      elementType: 'section',
      title: madxartwork.translate('all_content'),
      history: {
        behavior: this,
  ...
  saveCollectionHistory: function saveCollectionHistory(collection, event) {
    ...
    var title = madxartwork.history.history.getModelLabel(firstModel); // If it's an unknown model - don't save
  ...
  add: function add(models, toView, position) {
    ...
    } // Fix for case the iframe has been reloaded and the old `madxartwork-inner` is not exist.


    if (toView.$el.hasClass('madxartwork-inner') && toView.$el[0].ownerDocument !== madxartwork.$previewContents[0]) {
      toView = madxartwork.getPreviewView();
    }
...
module.exports = madxartworkModules.Module.extend({
  getDefaultSettings: function getDefaultSettings() {
    return {
      actions: {},
      classes: {
        list: 'madxartwork-context-menu-list',
        group: 'madxartwork-context-menu-list__group',
        groupPrefix: 'madxartwork-context-menu-list__group-',
        item: 'madxartwork-context-menu-list__item',
        itemTypePrefix: 'madxartwork-context-menu-list__item-',
        itemTitle: 'madxartwork-context-menu-list__item__title',
        itemShortcut: 'madxartwork-context-menu-list__item__shortcut',
        iconShortcut: 'madxartwork-context-menu-list__item__icon',
        itemDisabled: 'madxartwork-context-menu-list__item--disabled',
        divider: 'madxartwork-context-menu-list__divider',
  ...
  initModal: function initModal() {
    ...
    this.getModal = function () {
      if (!modal) {
        modal = madxartworkCommon.dialogsManager.createWidget('simple', {
          className: 'madxartwork-context-menu',
          message: this.buildActionsList(),
          iframe: madxartwork.$preview,
          effects: {
            hide: 'hide',
            show: 'show'
          },
          ...
          position: {
            my: (madxartworkCommon.config.isRTL ? 'right' : 'left') + ' top',
            collision: 'fit'
          }
        });
      }
...
function (_BaseAddSectionView) {
  ...
  (0, _createClass2.default)(AddSectionView, [{
    key: "className",
    value: function className() {
      return (0, _get2.default)((0, _getPrototypeOf2.default)(AddSectionView.prototype), "className", this).call(this) + ' madxartwork-add-section-inline';
    }
  }, {
    key: "fadeToDeath",
...
var BaseElementView = __webpack_require__(200),
    ColumnEmptyView = __webpack_require__(376),
    ColumnView;

ColumnView = BaseElementView.extend({
  template: Marionette.TemplateCache.get('#tmpl-madxartwork-column-content'),
  emptyView: ColumnEmptyView,
  childViewContainer: '> .madxartwork-column-wrap > .madxartwork-widget-wrap',
  ...
  behaviors: function behaviors() {
    ...
    _.extend(behaviors, {
      ...
      Resizable: {
        behaviorClass: __webpack_require__(377)
      }
    });

    return madxartwork.hooks.applyFilters('elements/column/behaviors', behaviors, this);
  },
  className: function className() {
    var classes = BaseElementView.prototype.className.apply(this, arguments),
        type = this.isInner() ? 'inner' : 'top';
    return classes + ' madxartwork-column madxartwork-' + type + '-column';
  },
  tagName: function tagName() {
  ...
  ui: function ui() {
    ...
    ui.columnInner = '> .madxartwork-column-wrap';
    ui.percentsTooltip = '> .madxartwork-element-overlay .madxartwork-column-percents-tooltip';
    return ui;
  },
  getEditButtons: function getEditButtons() {
    var elementData = madxartwork.getElementData(this.model),
  ...
  attachElContent: function attachElContent() {
    ...
    var $tooltip = jQuery('<div>', {
      class: 'madxartwork-column-percents-tooltip'
    });
    this.$el.children('.madxartwork-element-overlay').append($tooltip);
  },
  getContextMenuGroups: function getContextMenuGroups() {
    var groups = BaseElementView.prototype.getContextMenuGroups.apply(this, arguments),
    ...
    groups.splice(generalGroupIndex + 1, 0, {
      ...
      actions: [{
        name: 'addNew',
        icon: 'eicon-plus',
        title: madxartwork.translate('new_column'),
  ...
  getSortableOptions: function getSortableOptions() {
    return {
      connectWith: '.madxartwork-widget-wrap',
      items: '> .madxartwork-element'
    };
  },
  changeChildContainerClasses: function changeChildContainerClasses() {
    var emptyClass = 'madxartwork-element-empty',
        populatedClass = 'madxartwork-element-populated';
  ...
  onRender: function onRender() {
    ...
    self.$el.html5Droppable({
      items: ' > .madxartwork-column-wrap > .madxartwork-widget-wrap > .madxartwork-element, >.madxartwork-column-wrap > .madxartwork-widget-wrap > .madxartwork-empty-view > .madxartwork-first-add',
      axis: ['vertical'],
      groups: ['madxartwork-element'],
      isDroppingAllowed: self.isDroppingAllowed.bind(self),
      currentElementClass: 'madxartwork-html5dnd-current-element',
      placeholderClass: 'madxartwork-sortable-placeholder madxartwork-widget-placeholder',
      hasDraggingOnChildClass: 'madxartwork-dragging-on-child',
      onDropping: function onDropping(side, event) {
        ...
        madxartwork.getPreviewView().onPanelElementDragEnd();
...
module.exports = Marionette.ItemView.extend({
  template: '#tmpl-madxartwork-empty-preview',
  className: 'madxartwork-empty-view',
  events: {
    click: 'onClickAdd'
  },
  behaviors: function behaviors() {
  ...
  paste: function paste() {
    var self = this,
        elements = madxartworkCommon.storage.get('transfer').elements,
        index = 0;
    elements.forEach(function (item) {
...
ResizableBehavior = Marionette.Behavior.extend({
  defaults: {
    handles: madxartworkCommon.config.isRTL ? 'w' : 'e'
  },
  events: {
  ...
  initialize: function initialize() {
    ...
    this.listenTo(madxartwork.channels.dataEditMode, 'switch', this.onEditModeSwitched);
  },
...
var RevisionsCollection = __webpack_require__(380),
    RevisionsManager;
...
RevisionsManager = function RevisionsManager() {
  ...
  var attachEvents = function attachEvents() {
    madxartwork.channels.editor.on('saved', onEditorSaved);
  };
  ...
  this.requestRevisions = function (callback) {
    ...
    madxartworkCommon.ajax.addRequest('get_revisions', {
      success: function success(data) {
  ...
  this.setEditorData = function (data) {
    var collection = madxartwork.getRegion('sections').currentView.collection; // Don't track in history.
  ...
  this.getRevisionDataAsync = function (id, options) {
    ...
    return madxartworkCommon.ajax.addRequest('get_revision_data', options);
  };
  ...
  this.deleteRevision = function (revisionModel, options) {
    ...
    madxartworkCommon.ajax.addRequest('delete_revision', params);
  };
  ...
  (0, _createClass2.default)(_default, [{
    ...
    value: function navigate(up) {
      if (this.manager.getItems().length > 1) {
        madxartwork.getPanelView().getCurrentPageView().currentTab.navigate(up);
      }
    }
  }]);
  return _default;
...
var RevisionModel = __webpack_require__(381);
...
module.exports = Marionette.Behavior.extend({
  ...
  ui: function ui() {
    return {
      buttonPreview: '#madxartwork-panel-footer-saver-preview',
      buttonPublish: '#madxartwork-panel-saver-button-publish',
      buttonSaveOptions: '#madxartwork-panel-saver-button-save-options',
      buttonPublishLabel: '#madxartwork-panel-saver-button-publish-label',
      menuSaveDraft: '#madxartwork-panel-footer-sub-menu-item-save-draft',
      lastEditedWrapper: '.madxartwork-last-edited-wrapper'
    };
  },
  events: function events() {
    return {
  ...
  initialize: function initialize() {
    madxartwork.saver.on('before:save', this.onBeforeSave.bind(this)).on('after:save', this.onAfterSave.bind(this)).on('after:saveError', this.onAfterSaveError.bind(this)).on('page:status:change', this.onPageStatusChange);
    madxartwork.settings.page.model.on('change', this.onPageSettingsChange.bind(this));
    madxartwork.channels.editor.on('status:change', this.activateSaveButtons.bind(this));
  },
  activateSaveButtons: function activateSaveButtons(hasChanges) {
    hasChanges = hasChanges || 'draft' === madxartwork.settings.page.model.get('post_status');
    this.ui.buttonPublish.add(this.ui.menuSaveDraft).toggleClass('madxartwork-disabled', !hasChanges);
    this.ui.buttonSaveOptions.toggleClass('madxartwork-disabled', !hasChanges);
  },
  onRender: function onRender() {
    this.setMenuItems(madxartwork.settings.page.model.get('post_status'));
    this.addTooltip();
  },
  ...
  onPageStatusChange: function onPageStatusChange(newStatus) {
    if ('publish' === newStatus) {
      madxartwork.notifications.showToast({
        message: madxartwork.config.document.panel.messages.publish_notification,
        buttons: [{
          name: 'view_page',
          text: madxartwork.translate('have_a_look'),
          callback: function callback() {
            open(madxartwork.config.document.urls.permalink);
          }
        }]
      });
    }
  ...
  onBeforeSave: function onBeforeSave(options) {
    ...
    if ('autosave' === options.status) {
      this.ui.lastEditedWrapper.addClass('madxartwork-state-active');
    } else {
      this.ui.buttonPublish.addClass('madxartwork-button-state');
    }
  },
  onAfterSave: function onAfterSave(data) {
    NProgress.done();
    this.ui.buttonPublish.removeClass('madxartwork-button-state');
    this.ui.lastEditedWrapper.removeClass('madxartwork-state-active');
    this.refreshWpPreview();
    this.setLastEdited(data);
  },
  setLastEdited: function setLastEdited(data) {
    this.ui.lastEditedWrapper.removeClass('madxartwork-button-state').find('.madxartwork-last-edited').html(data.config.document.last_edited);
  },
  ...
  onClickButtonPreview: function onClickButtonPreview() {
    ...
    this.previewWindow = open(madxartwork.config.document.urls.wp_preview, 'wp-preview-' + madxartwork.config.document.id);

    if (madxartwork.saver.isEditorChanged()) {
      // Force save even if it's saving now.
      if (madxartwork.saver.isSaving) {
        madxartwork.saver.isSaving = false;
      }

      madxartwork.saver.doAutoSave();
    }
  },
  onClickButtonPublish: function onClickButtonPublish() {
    if (this.ui.buttonPublish.hasClass('madxartwork-disabled')) {
      return;
    }
  ...
  setMenuItems: function setMenuItems(postStatus) {
    ...
    switch (postStatus) {
      ...
      case 'draft':
        if (!madxartwork.config.current_user_can_publish) {
          publishLabel = 'submit';
        }
    ...
    this.ui.buttonPublishLabel.html(madxartwork.translate(publishLabel));
  },
  addTooltip: function addTooltip() {
    // Create tooltip on controls
  ...
  refreshWpPreview: function refreshWpPreview() {
    if (this.previewWindow) {
      ...
      try {
        this.previewWindow.location.href = madxartwork.config.document.urls.wp_preview;
...
module.exports = ControlBaseView.extend({
  ...
  onButtonClick: function onButtonClick() {
    ...
    madxartwork.channels.editor.trigger(eventName, this);
  }
});
...
ControlCodeEditorItemView = ControlBaseDataView.extend({
  ui: function ui() {
    var ui = ControlBaseDataView.prototype.ui.apply(this, arguments);
    ui.editor = '.madxartwork-code-editor';
    return ui;
  },
  onReady: function onReady() {
    var self = this;
    ...
    jQuery(self.editor.container).addClass('madxartwork-input-style madxartwork-code-editor');
    ...
    madxartwork.panel.$el.on('resize.aceEditor', self.onResize.bind(this));
  ...
  onDestroy: function onDestroy() {
    madxartwork.panel.$el.off('resize.aceEditor');
  }
});
...
ControlColorItemView = ControlBaseDataView.extend({
  applySavedValue: function applySavedValue() {
    ...
    if (colorInstance) {
      ...
      madxartwork.helpers.wpColorPicker(self.ui.input, {
        change: function change() {
...
ControlDimensionsItemView = ControlBaseUnitsItemView.extend({
  ui: function ui() {
    ...
    ui.controls = '.madxartwork-control-dimension > input:enabled';
    ui.link = 'button.madxartwork-link-dimensions';
    return ui;
  },
  events: function events() {
...
__webpack_require__(388)('link', function (createHTML) {
...
ControlMediaItemView = ControlBaseDataView.extend({
  ui: function ui() {
    var ui = ControlBaseDataView.prototype.ui.apply(this, arguments);
    ui.addImages = '.madxartwork-control-gallery-add';
    ui.clearGallery = '.madxartwork-control-gallery-clear';
    ui.galleryThumbnails = '.madxartwork-control-gallery-thumbnails';
    ui.status = '.madxartwork-control-gallery-status-title';
    return ui;
  },
  events: function events() {
    return _.extend(ControlBaseDataView.prototype.events.apply(this, arguments), {
  ...
  applySavedValue: function applySavedValue() {
    ...
    this.$el.toggleClass('madxartwork-gallery-has-images', hasImages).toggleClass('madxartwork-gallery-empty', !hasImages);
    var $galleryThumbnails = this.ui.galleryThumbnails;
    $galleryThumbnails.empty();
    this.ui.status.text(madxartwork.translate(hasImages ? 'gallery_images_selected' : 'gallery_no_images_selected', [imagesCount]));
    ...
    this.getControlValue().forEach(function (image) {
      var $thumbnail = jQuery('<div>', {
        class: 'madxartwork-control-gallery-thumbnail'
      });
  ...
  initFrame: function initFrame(action) {
    ...
    var options = {
      ...
      button: {
        text: madxartwork.translate('insert_media')
      }
    };
  ...
  initRemoveDialog: function initRemoveDialog() {
    ...
    this.getRemoveDialog = function () {
      if (!removeDialog) {
        removeDialog = madxartworkCommon.dialogsManager.createWidget('confirm', {
          message: madxartwork.translate('dialog_confirm_gallery_delete'),
          headerMessage: madxartwork.translate('delete_gallery'),
          strings: {
            confirm: madxartwork.translate('delete'),
            cancel: madxartwork.translate('cancel')
          },
...
function (_ControlMultipleBaseI) {
  ...
  (0, _createClass2.default)(ControlIconsView, [{
    ...
    value: function enqueueIconFonts(iconType) {
      var iconSetting = madxartwork.helpers.getIconLibrarySettings(iconType);
      ...
      if (iconSetting.enqueue) {
        iconSetting.enqueue.forEach(function (assetURL) {
          madxartwork.helpers.enqueueEditorStylesheet(assetURL);
          madxartwork.helpers.enqueuePreviewStylesheet(assetURL);
        });
      }

      if (iconSetting.url) {
        madxartwork.helpers.enqueueEditorStylesheet(iconSetting.url);
        madxartwork.helpers.enqueuePreviewStylesheet(iconSetting.url);
      }
    }
  }, {
    key: "ui",
    value: function ui() {
      ...
      ui.controlMedia = '.madxartwork-control-media';
      ui.svgUploader = '.madxartwork-control-svg-uploader';
      ui.iconPickers = '.madxartwork-control-icon-picker, .madxartwork-control-media__preview, .madxartwork-control-media-upload-button';
      ui.deleteButton = '.madxartwork-control-media__remove';
      ui.previewPlaceholder = '.madxartwork-control-media__preview';
      ui.previewContainer = '.madxartwork-control-preview-area';
      return ui;
    }
  }, {
    key: "events",
    ...
    value: function migrateFa4toFa5(fa4Value) {
      var fa5Value = madxartwork.helpers.mapFa4ToFa5(fa4Value);
    ...
    value: function isMigrationAllowed() {
      return !madxartworkConfig['icons_update_needed'];
    }
  }, {
    ...
    value: function onReady() {
      ...
      if (!this.isMigrationAllowed()) {
        this.ui.previewContainer[0].addEventListener('click', function (event) {
          ...
          var enableMigrationDialog = madxartwork.helpers.getSimpleDialog('madxartwork-enable-fa5-dialog', madxartwork.translate('enable_fa5'), madxartwork.translate('dialog_confirm_enable_fa5'), madxartwork.translate('update'), onConfirm);
    ...
    value: function initFrame() {
      ...
      wp.media.view.settings.post.id = madxartwork.config.document.id;
      this.frame = wp.media({
        button: {
          text: madxartwork.translate('insert_media')
        },
        ...
        states: [new wp.media.controller.Library({
          title: madxartwork.translate('insert_media'),
    ...
    value: function getSvgNotEnabledDialog() {
      ...
      var onConfirm = function onConfirm() {
        madxartworkCommon.ajax.addRequest('enable_svg_uploads', {}, true);

        _this4.openFrame();
      };

      return madxartwork.helpers.getSimpleDialog('madxartwork-enable-svg-dialog', madxartwork.translate('enable_svg'), madxartwork.translate('dialog_confirm_enable_svg'), madxartwork.translate('enable'), onConfirm);
    }
  }, {
    key: "isSvgEnabled",
    ...
    value: function openFrame() {
      if (!this.isSvgEnabled() && !madxartwork.iconManager.cache.svgDialogShown) {
        var dialog = this.getSvgNotEnabledDialog();
        madxartwork.iconManager.cache.svgDialogShown = true;
        return dialog.show();
      }
      ...
      this.frame.uploader.uploader.param('uploadTypeCaller', 'madxartwork-editor-upload');
    ...
    value: function applySavedValue() {
      ...
      this.ui.controlMedia.toggleClass('madxartwork-media-empty', !iconValue);
      ...
      if ('svg' === iconType) {
        return madxartwork.helpers.fetchInlineSvg(iconValue.url, function (data) {
...
ControlImageDimensionsItemView = ControlMultipleBaseItemView.extend({
  ui: function ui() {
    return {
      ...
      btnApply: 'button.madxartwork-image-dimensions-apply-button'
    };
  },
...
ControlMediaItemView = ControlMultipleBaseItemView.extend({
  ui: function ui() {
    var ui = ControlMultipleBaseItemView.prototype.ui.apply(this, arguments);
    ui.controlMedia = '.madxartwork-control-media';
    ui.mediaImage = '.madxartwork-control-media__preview';
    ui.mediaVideo = '.madxartwork-control-media-video';
    ui.frameOpeners = '.madxartwork-control-preview-area';
    ui.removeButton = '.madxartwork-control-media__remove';
    return ui;
  },
  events: function events() {
  ...
  applySavedValue: function applySavedValue() {
    ...
    this.ui.controlMedia.toggleClass('madxartwork-media-empty', !url);
  },
...
var ControlBaseDataView = __webpack_require__(97),
    NumberValidator = __webpack_require__(397),
...
ControlOrderItemView = ControlMultipleBaseItemView.extend({
  ui: function ui() {
    var ui = ControlMultipleBaseItemView.prototype.ui.apply(this, arguments);
    ui.reverseOrderLabel = '.madxartwork-control-order-label';
    return ui;
  },
  changeLabelTitle: function changeLabelTitle() {
    var reverseOrder = this.getControlValue('reverse_order');
    this.ui.reverseOrderLabel.attr('title', madxartwork.translate(reverseOrder ? 'asc' : 'desc'));
  },
  onRender: function onRender() {
...
ControlPopoverStarterView = ControlChooseView.extend({
  ui: function ui() {
    ...
    ui.popoverToggle = '.madxartwork-control-popover-toggle-toggle';
    return ui;
  },
  events: function events() {
  ...
  onPopoverToggleClick: function onPopoverToggleClick() {
    this.$el.next('.madxartwork-controls-popover').toggle();
  }
}, {
  onPasteStyle: function onPasteStyle(control, clipboardValue) {
...
ControlRepeaterItemView = ControlBaseDataView.extend({
  ui: {
    btnAddRow: '.madxartwork-repeater-add',
    fieldContainer: '.madxartwork-repeater-fields-wrapper'
  },
  events: function events() {
    return {
  ...
  childViewContainer: '.madxartwork-repeater-fields-wrapper',
  templateHelpers: function templateHelpers() {
    return {
  ...
  createItemModel: function createItemModel(attrs, options, controlView) {
    ...
    if (!attrs._id) {
      attrs._id = madxartwork.helpers.getUniqueID();
    }

    return new madxartworkModules.editor.elements.models.BaseSettings(attrs, options);
  },
  ...
  addRow: function addRow(data, options) {
    var id = madxartwork.helpers.getUniqueID();
  ...
  editRow: function editRow(rowView) {
    if (this.currentEditableChild) {
      ...
      currentEditable.find('.madxartwork-wp-editor').each(function () {
  ...
  toggleMinRowsClass: function toggleMinRowsClass() {
    ...
    this.$el.toggleClass('madxartwork-repeater-has-minimum-rows', 1 >= this.collection.length);
  },
  ...
  onRender: function onRender() {
    ...
    if (this.model.get('item_actions').sort) {
      this.ui.fieldContainer.sortable({
        axis: 'y',
        handle: '.madxartwork-repeater-row-tools'
      });
    }
...
ControlSectionItemView = ControlBaseView.extend({
  ui: function ui() {
    var ui = ControlBaseView.prototype.ui.apply(this, arguments);
    ui.heading = '.madxartwork-panel-heading';
    return ui;
  },
  triggers: {
...
ControlSliderItemView = ControlBaseUnitsItemView.extend({
  ...
  initSlider: function initSlider() {
    ...
    var isMultiple = this.isMultiple(),
        unitRange = madxartworkCommon.helpers.cloneObject(this.getCurrentRange()),
        step = unitRange.step;
  ...
  onReady: function onReady() {
    if (this.isMultiple()) {
      this.$el.addClass('madxartwork-control-type-slider--multiple madxartwork-control-type-slider--handles-' + this.model.get('handles'));
    }
  ...
  onSlideChange: function onSlideChange(values, index) {
    if (this.isMultiple()) {
      var sizes = madxartworkCommon.helpers.cloneObject(this.getSize()),
...
ControlStructureItemView = ControlBaseDataView.extend({
  ui: function ui() {
    var ui = ControlBaseDataView.prototype.ui.apply(this, arguments);
    ui.resetStructure = '.madxartwork-control-structure-reset';
    return ui;
  },
  events: function events() {
    return _.extend(ControlBaseDataView.prototype.events.apply(this, arguments), {
  ...
  getCurrentEditedSection: function getCurrentEditedSection() {
    var editor = madxartwork.getPanelView().getCurrentPageView();
    return editor.getOption('editedElementView');
  },
  getMorePresets: function getMorePresets() {
    var parsedStructure = madxartwork.presetsFactory.getParsedStructure(this.getControlValue());
    return madxartwork.presetsFactory.getPresets(parsedStructure.columnsCount);
  },
  onInputChange: function onInputChange() {
...
module.exports = BaseMultiple.extend({
  onReady: function onReady() {
    var self = this,
        positionBase = madxartworkCommon.config.isRTL ? 'right' : 'left',
        last,
    ...
    this.ui.input.autocomplete({
      ...
      open: function open(event) {
        jQuery(event.target).data('uiAutocomplete').menu.activeMenu.addClass('madxartwork-autocomplete-menu');
      },
      minLength: 2,
      position: {
    ...
    .autocomplete('instance')._renderItem = function (ul, item) {
      ...
      return jQuery('<li role="option" id="mce-wp-autocomplete-' + item.ID + '">').append('<span>' + title + '</span>&nbsp;<span class="madxartwork-autocomplete-item-info">' + item.info + '</span>').appendTo(ul);
    };
  },
  onBeforeDestroy: function onBeforeDestroy() {
...
ControlWPWidgetItemView = ControlBaseDataView.extend({
  ...
  onFormChanged: function onFormChanged() {
    var idBase = 'widget-' + this.model.get('id_base'),
        settings = this.ui.form.madxartworkSerializeObject()[idBase].REPLACE_TO_ID;
    this.setValue(settings);
  },
  onReady: function onReady() {
    var self = this;
    madxartworkCommon.ajax.addRequest('editor_get_wp_widget_form', {
      data: {
        // Fake Widget ID
        id: self.model.cid,
      ...
      success: function success(data) {
        ...
        madxartwork.hooks.doAction('panel/widgets/' + self.model.get('widget') + '/controls/wp_widget/loaded', self);
      }
    });
  }
});
...
ControlWysiwygItemView = ControlBaseDataView.extend({
  ...
  ui: function ui() {
    var ui = ControlBaseDataView.prototype.ui.apply(this, arguments);
    jQuery.extend(ui, {
      inputWrapper: '.madxartwork-control-input-wrapper'
    });
    return ui;
  },
  events: function events() {
    return _.extend(ControlBaseDataView.prototype.events.apply(this, arguments), {
      'keyup textarea.madxartwork-wp-editor': 'onBaseInputChange'
    });
  },
  ...
  initialize: function initialize() {
    ControlBaseDataView.prototype.initialize.apply(this, arguments);
    var self = this;
    self.editorID = 'madxartworkwpeditor' + self.cid; // Wait a cycle before initializing the editors.
    ...
    _.defer(function () {
      ...
      if (madxartwork.config.rich_editing_enabled) {
    ...
    if (!madxartwork.config.rich_editing_enabled) {
      self.$el.addClass('madxartwork-rich-editing-disabled');
      return;
    }
    ...
    tinyMCEPreInit.mceInit[self.editorID] = _.extend(_.clone(tinyMCEPreInit.mceInit.madxartworkwpeditor), editorConfig);

    if (!madxartwork.config.tinymceHasCustomConfig) {
  ...
  onReady: function onReady() {
    ...
    var $editor = jQuery(madxartwork.config.wp_editor.replace(/madxartworkwpeditor/g, this.editorID).replace('%%EDITORCONTENT%%', this.getControlValue()));
...
module.exports = madxartworkModules.editor.elements.models.BaseSettings.extend({
  defaults: {
    _column_size: 100
  }
});
...
var _widgetDraggable = _interopRequireDefault(__webpack_require__(412));

var _widgetResizeable = _interopRequireDefault(__webpack_require__(413));
...
WidgetView = BaseElementView.extend({
  ...
  getTemplate: function getTemplate() {
    ...
    if ('remote' !== this.getTemplateType()) {
      return Marionette.TemplateCache.get('#tmpl-madxartwork-' + editModel.get('widgetType') + '-content');
    }
  ...
  className: function className() {
    ...
    return baseClasses + ' madxartwork-widget ' + madxartwork.getElementData(this.getEditModel()).html_wrapper_class;
  },
  events: function events() {
  ...
  behaviors: function behaviors() {
    ...
    _.extend(behaviors, {
      InlineEditing: {
        behaviorClass: __webpack_require__(414),
        inlineEditingClass: 'madxartwork-inline-editing'
      },
      Draggable: {
    ...
    return madxartwork.hooks.applyFilters('elements/widget/behaviors', behaviors, this);
  },
  getEditButtons: function getEditButtons() {
    var elementData = madxartwork.getElementData(this.model),
        editTools = {};
  ...
  getContextMenuGroups: function getContextMenuGroups() {
    ...
    groups.splice(transferGroupIndex + 1, 0, {
      ...
      actions: [{
        name: 'save',
        title: madxartwork.translate('save_as_global'),
        shortcut: jQuery('<i>', {
  ...
  render: function render() {
    if (this.model.isRemoteRequestActive()) {
      ...
      this.$el.addClass('madxartwork-element');
      return;
    }
  ...
  handleEmptyWidget: function handleEmptyWidget() {
    // TODO: REMOVE THIS !!
    // TEMP CODING !!
    this.$el.addClass('madxartwork-widget-empty').append('<i class="madxartwork-widget-empty-icon ' + this.getEditModel().getIcon() + '"></i>');
  },
  getTemplateType: function getTemplateType() {
    if (null === this._templateType) {
      var editModel = this.getEditModel(),
          $template = jQuery('#tmpl-madxartwork-' + editModel.get('widgetType') + '-content');
  ...
  attachElContent: function attachElContent(html) {
    ...
    _.defer(function () {
      madxartworkFrontend.elements.window.jQuery(_this.el).empty().append(_this.getHandlesOverlay(), _this.getHTMLContent(html));
  ...
  addInlineEditingAttributes: function addInlineEditingAttributes(key, toolbar) {
    this.addRenderAttribute(key, {
      class: 'madxartwork-inline-editing',
      'data-madxartwork-setting-key': key
    });

    if (toolbar) {
      this.addRenderAttribute(key, {
        'data-madxartwork-inline-editing-toolbar': toolbar
      });
    }
  },
  ...
  onModelBeforeRemoteRender: function onModelBeforeRemoteRender() {
    this.$el.addClass('madxartwork-loading');
  },
  onBeforeDestroy: function onBeforeDestroy() {
    // Remove old style from the DOM.
    madxartwork.$previewContents.find('#madxartwork-style-' + this.model.cid).remove();
  },
  onModelRemoteRender: function onModelRemoteRender() {
    ...
    this.$el.removeClass('madxartwork-loading');
    this.render();
  },
  onRender: function onRender() {
    var self = this;
    ...
    self.$el.attr('data-widget_type', editModel.get('widgetType') + '.' + skinType).removeClass('madxartwork-widget-empty').children('.madxartwork-widget-empty-icon').remove(); // TODO: Find a better way to detect if all the images have been loaded

    self.$el.imagesLoaded().always(function () {
      setTimeout(function () {
        if (1 > self.$el.children('.madxartwork-widget-container').outerHeight()) {
...
function (_Marionette$Behavior) {
  ...
  (0, _createClass2.default)(_default, [{
    ...
    value: function initialize() {
      ...
      this.listenTo(madxartwork.channels.dataEditMode, 'switch', this.toggle);
      var view = this.view,
    ...
    value: function toggle() {
      var isEditMode = 'edit' === madxartwork.channels.dataEditMode.request('activeMode'),
          isAbsolute = this.view.getEditModel().getSetting('_position');
      this.deactivate();

      if (isEditMode && isAbsolute && madxartwork.userCan('design')) {
        this.activate();
      }
    }
  }, {
    ...
    value: function onDragStop(event, ui) {
      ...
      var currentDeviceMode = madxartworkFrontend.getCurrentDeviceMode(),
          ...
          settingToChange = {},
          isRTL = madxartworkFrontend.config.is_rtl;
      ...
      xPos = madxartwork.helpers.elementSizeToUnit(this.$el, xPos, offsetXUnit);
      ...
      yPos = madxartwork.helpers.elementSizeToUnit(this.$el, yPos, offsetYUnit);
  ...
  (0, _createClass2.default)(_default, [{
    ...
    value: function toggle() {
      var editModel = this.view.getEditModel(),
          isEditMode = 'edit' === madxartwork.channels.dataEditMode.request('activeMode'),
          isAbsolute = editModel.getSetting('_position'),
          isInline = 'initial' === editModel.getSetting('_element_width');
      this.deactivate();

      if (isEditMode && (isAbsolute || isInline) && madxartwork.userCan('design')) {
        this.activate();
      }
    }
  }, {
    ...
    value: function onResizeStop(event, ui) {
      ...
      var currentDeviceMode = madxartworkFrontend.getCurrentDeviceMode(),
          ...
          width = madxartwork.helpers.elementSizeToUnit(this.$el, ui.size.width, unit),
          settingToChange = {};
...
InlineEditingBehavior = Marionette.Behavior.extend({
  ...
  getEditingSettingKey: function getEditingSettingKey() {
    return this.$currentEditingArea.data().madxartworkSettingKey;
  },
  startEditing: function startEditing($element) {
    if (this.editing || 'edit' !== madxartwork.channels.dataEditMode.request('activeMode') || this.view.model.isRemoteRequestActive()) {
      return;
    }

    var madxartworkSettingKey = $element.data().madxartworkSettingKey,
        settingKey = madxartworkSettingKey,
        keyParts = madxartworkSettingKey.split('.'),
        isRepeaterKey = 3 === keyParts.length,
    ...
    var elementData = this.$currentEditingArea.data(),
        elementDataToolbar = elementData.madxartworkInlineEditingToolbar,
        mode = 'advanced' === elementDataToolbar ? 'advanced' : 'basic',
        editModel = this.view.getEditModel(),
        inlineEditingConfig = madxartwork.config.inlineEditing,
    ...
    var madxartworkInlineEditor = madxartworkFrontend.elements.window.madxartworkInlineEditor;
    this.editing = true;
    this.view.allowRender = false; // Avoid retrieving of old content (e.g. in case of sorting)

    this.view.model.setHtmlCache('');
    this.editor = new madxartworkInlineEditor({
      ...
      placeholder: madxartwork.translate('type_here') + '...',
    ...
    madxartworkCommon.elements.$body.on('mousedown', this.onInlineEditingBlur);
  },
  stopEditing: function stopEditing() {
    this.editing = false;
    this.$currentEditingArea.off('blur', this.onInlineEditingBlur);
    madxartworkCommon.elements.$body.off('mousedown', this.onInlineEditingBlur);
    this.editor.destroy();
    ...
    if ('advanced' === this.$currentEditingArea.data().madxartworkInlineEditingToolbar) {
  ...
  onInlineEditingBlur: function onInlineEditingBlur(event) {
    ...
    setTimeout(function () {
      var selection = madxartworkFrontend.elements.window.getSelection(),
...
PanelElementsElementsView = Marionette.CollectionView.extend({
  ...
  id: 'madxartwork-panel-elements',
  initialize: function initialize() {
    this.listenTo(madxartwork.channels.panelElements, 'filter:change', this.onFilterChanged);
  },
  filter: function filter(childModel) {
    var filterValue = madxartwork.channels.panelElements.request('filter:value');
...
var PanelMenuGroupView = __webpack_require__(417),
    PanelMenuPageView;

PanelMenuPageView = Marionette.CompositeView.extend({
  id: 'madxartwork-panel-page-menu',
  template: '#tmpl-madxartwork-panel-menu',
  childView: PanelMenuGroupView,
  childViewContainer: '#madxartwork-panel-page-menu-content',
  initialize: function initialize() {
    this.collection = PanelMenuPageView.getGroups();
  },
  getArrowClass: function getArrowClass() {
    return 'eicon-arrow-' + (madxartworkCommon.config.isRTL ? 'right' : 'left');
  },
  onRender: function onRender() {
    madxartwork.getPanelView().getHeaderView().ui.menuIcon.removeClass('eicon-menu-bar').addClass(this.getArrowClass());
  },
  onDestroy: function onDestroy() {
    madxartwork.getPanelView().getHeaderView().ui.menuIcon.removeClass(this.getArrowClass()).addClass('eicon-menu-bar');
  }
}, {
  groups: null,
  initGroups: function initGroups() {
    ...
    this.groups.add({
      name: 'go_to',
      title: madxartwork.translate('go_to'),
      items: []
    });
    this.addItem({
      name: 'view-page',
      icon: 'eicon-preview',
      title: madxartwork.translate('view_page'),
      type: 'link',
      link: madxartwork.config.document.urls.permalink
    }, 'go_to');
    this.addItem({
      name: 'exit-to-dashboard',
      icon: 'eicon-wordpress',
      title: madxartwork.translate('exit_to_dashboard'),
      type: 'link',
      link: madxartwork.config.document.urls.exit_to_dashboard
    }, 'go_to');

    if (madxartwork.config.user.is_administrator) {
      this.addAdminMenu();
    }
  },
  addAdminMenu: function addAdminMenu() {
    this.groups.add({
      name: 'style',
      title: madxartwork.translate('global_style'),
      items: []
    }, {
      at: 0
    });
    this.groups.add({
      name: 'settings',
      title: madxartwork.translate('settings'),
      items: []
    }, {
      at: 1
    });
    this.addItem({
      ...
      title: madxartworkCommon.translate('finder', 'finder'),
      callback: function callback() {
    ...
    this.addItem({
      ...
      title: madxartwork.translate('global_colors'),
      type: 'page',
      callback: function callback() {
    ...
    this.addItem({
      name: 'global-fonts',
      icon: 'eicon-font',
      title: madxartwork.translate('global_fonts'),
      type: 'page',
      callback: function callback() {
    ...
    this.addItem({
      ...
      title: madxartwork.translate('color_picker'),
      type: 'page',
      callback: function callback() {
    ...
    this.addItem({
      ...
      icon: 'eicon-cogs',
      title: madxartwork.translate('global_settings'),
      type: 'page',
      callback: function callback() {
        return $e.route('panel/general-settings/style');
      }
    }, 'settings', 'madxartwork-settings');
    this.addItem({
      name: 'madxartwork-settings',
      icon: 'eicon-editor-external-link',
      title: madxartwork.translate('madxartwork_settings'),
      type: 'link',
      link: madxartwork.config.settings_page_link,
      newTab: true
    }, 'settings');
    this.addItem({
      name: 'about-madxartwork',
      icon: 'eicon-info-circle',
      title: madxartwork.translate('about_madxartwork'),
      type: 'link',
      link: madxartwork.config.madxartwork_site,
      newTab: true
    }, 'settings');
  },
...
var PanelMenuItemView = __webpack_require__(418);

module.exports = Marionette.CompositeView.extend({
  template: '#tmpl-madxartwork-panel-menu-group',
  className: 'madxartwork-panel-menu-group',
  childView: PanelMenuItemView,
  childViewContainer: '.madxartwork-panel-menu-items',
  initialize: function initialize() {
...
module.exports = Marionette.ItemView.extend({
  template: '#tmpl-madxartwork-panel-menu-item',
  className: function className() {
    return 'madxartwork-panel-menu-item madxartwork-panel-menu-item-' + this.model.get('name');
  },
  triggers: {
    click: {
      event: 'click',
...
module.exports = madxartworkModules.Module.extend({
  ...
  loadCacheRequests: function loadCacheRequests() {
    ...
    this.cacheRequests = {};
    this.cacheCallbacks = [];
    madxartworkCommon.ajax.addRequest('render_tags', {
      data: {
        post_id: madxartwork.config.document.id,
  ...
  getConfig: function getConfig(key) {
    return this.getItems(madxartwork.config.dynamicTags, key);
  },
  parseTagsText: function parseTagsText(text, settings, parseCallback) {
    ...
    return text.replace(/\[madxartwork-tag[^\]]+]/g, function (tagText) {
  ...
  createTag: function createTag(tagID, tagName, tagSettings) {
    ...
    var TagClass = this.tags[tagName] || this.tags.Base,
        model = new madxartworkModules.editor.elements.models.BaseSettings(tagSettings, {
  ...
  tagDataToTagText: function tagDataToTagText(tagID, tagName, tagSettings) {
    ...
    return '[madxartwork-tag id="' + tagID + '" name="' + tagName + '" settings="' + tagSettings + '"]';
  },
...
module.exports = Marionette.ItemView.extend({
  ...
  getTemplate: function getTemplate() {
    ...
    return Marionette.TemplateCache.get('#tmpl-madxartwork-tag-' + this.getOption('name') + '-content');
  },
  initialize: function initialize() {
    try {
      this.getTemplate();
  ...
  getConfig: function getConfig(key) {
    var config = madxartwork.dynamicTags.getConfig('tags.' + this.getOption('name'));
  ...
  getContent: function getContent() {
    ...
    if (!this.hasTemplate) {
      data = madxartwork.dynamicTags.loadTagDataFromCache(this);

      if (undefined === data) {
        throw new Error(madxartwork.dynamicTags.CACHE_KEY_NOT_FOUND_ERROR);
      }
    }
  ...
  onRender: function onRender() {
    this.el.id = 'madxartwork-tag-' + this.getOption('id');
  }
});
...
module.exports = madxartworkModules.Module.extend({
  modules: {
    ...
    general: __webpack_require__(422),
    page: __webpack_require__(424)
  },
  panelPages: {
    base: __webpack_require__(426)
  },
  onInit: function onInit() {
    this.initSettings();
  },
  initSettings: function initSettings() {
    ...
    _.each(madxartwork.config.settings, function (config, name) {
...
var _component = _interopRequireDefault(__webpack_require__(423));
...
module.exports = BaseSettings.extend({
  ...
  changeCallbacks: {
    madxartwork_page_title_selector: function madxartwork_page_title_selector(newValue) {
      var newSelector = newValue || 'h1.entry-title',
          titleSelectors = madxartwork.settings.page.model.controls.hide_title.selectors = {};
      titleSelectors[newSelector] = 'display: none';
      madxartwork.settings.page.updateStylesheet();
    }
  }
});
...
function (_madxartworkModules$com) {
  ...
  (0, _createClass2.default)(_default, [{
    ...
    value: function defaultTabs() {
      return {
        style: {
          title: madxartwork.translate('style')
        },
        lightbox: {
          title: madxartwork.translate('lightbox')
        }
      };
    }
  }, {
    key: "renderTab",
    value: function renderTab(tab) {
      madxartwork.getPanelView().setPage('general_settings').activateTab(tab);
    }
  }]);
  return _default;
}(madxartworkModules.common.Component);
...
var _component = _interopRequireDefault(__webpack_require__(425));
...
module.exports = BaseSettings.extend({
  ...
  changeCallbacks: {
    post_title: function post_title(newValue) {
      var $title = madxartworkFrontend.elements.$document.find(madxartwork.config.page_title_selector);
      $title.text(newValue);
    },
  ...
  getDataToSave: function getDataToSave(data) {
    data.id = madxartwork.config.document.id;
    return data;
  }
});
  ...
  (0, _createClass2.default)(_default, [{
    ...
    value: function defaultTabs() {
      return {
        ...
        advanced: {
          title: madxartwork.translate('advanced')
        }
      };
    }
  }, {
    key: "renderTab",
    value: function renderTab(tab) {
      madxartwork.getPanelView().setPage('page_settings').activateTab(tab);
    }
  }, {
    key: "getTabsWrapperSelector",
    value: function getTabsWrapperSelector() {
      return '.madxartwork-panel-navigation';
    }
  }]);
  return _default;
}(madxartworkModules.common.Component);
...
module.exports = madxartworkModules.editor.views.ControlsStack.extend({
  id: function id() {
    return 'madxartwork-panel-' + this.getOption('name') + '-settings';
  },
  getTemplate: function getTemplate() {
    return '#tmpl-madxartwork-panel-' + this.getOption('name') + '-settings';
  },
  childViewContainer: function childViewContainer() {
    return '#madxartwork-panel-' + this.getOption('name') + '-settings-controls';
  },
  childViewOptions: function childViewOptions() {
    return {
      elementSettingsModel: this.model
...
module.exports = madxartworkModules.Module.extend({
  autoSaveTimer: null,
  autosaveInterval: madxartwork.config.autosave_interval * 1000,
  isSaving: false,
  ...
  saveDraft: function saveDraft() {
    var postStatus = madxartwork.settings.page.model.get('post_status');

    if (!madxartwork.saver.isEditorChanged() && 'draft' !== postStatus) {
      return;
    }
  ...
  doAutoSave: function doAutoSave() {
    var editorMode = madxartwork.channels.dataEditMode.request('activeMode'); // Avoid auto save for Revisions Preview changes.
  ...
  discard: function discard() {
    var self = this;
    madxartworkCommon.ajax.addRequest('discard_changes', {
      success: function success() {
        self.setFlagEditorChange(false);
        location.href = madxartwork.config.document.urls.exit_to_dashboard;
      }
    });
  },
  update: function update(options) {
    options = _.extend({
      status: madxartwork.settings.page.model.get('post_status')
    }, options);
    this.saveEditor(options);
  },
  ...
  setFlagEditorChange: function setFlagEditorChange(status) {
    ...
    madxartwork.channels.editor.reply('status', status).trigger('status:change', status);
  },
  isEditorChanged: function isEditorChanged() {
    return true === madxartwork.channels.editor.request('status');
  },
  setWorkSaver: function setWorkSaver() {
    var self = this;
    madxartworkCommon.elements.$window.on('beforeunload', function () {
      if (self.isEditorChanged()) {
        return madxartwork.translate('before_unload_alert');
      }
    });
  },
  ...
  saveEditor: function saveEditor(options) {
    ...
    var self = this,
        elements = madxartwork.elements.toJSON({
      remove: ['default', 'editSettings', 'defaultEditSettings']
    }),
        settings = madxartwork.settings.page.model.toJSON({
      remove: ['default']
    }),
        oldStatus = madxartwork.settings.page.model.get('post_status'),
    ...
    madxartworkCommon.ajax.addRequest('save_builder', {
      data: {
      ...
      success: function success(data) {
        ...
        if ('autosave' !== options.status) {
          if (statusChanged) {
            madxartwork.settings.page.model.set('post_status', options.status);
        ...
        if (data.config) {
          jQuery.extend(true, madxartwork.config, data.config);
        }

        madxartwork.config.data = elements;
        madxartwork.channels.editor.trigger('saved', data);
      ...
      error: function error(data) {
        ...
        if (_.isString(data)) {
          ...
          message = madxartwork.createAjaxErrorMessage(data);

          if (0 === data.readyState) {
            message += ' ' + madxartwork.translate('saving_disabled');
          }
        } else if (data[0] && data[0].code) {
          message = madxartwork.translate('server_error') + ' ' + data[0].code;
        }
...
module.exports = madxartworkModules.Module.extend({
  initToast: function initToast() {
    var toast = madxartworkCommon.dialogsManager.createWidget('buttons', {
      id: 'madxartwork-toast',
      position: {
        my: 'center bottom',
        at: 'center bottom-10',
        of: '#madxartwork-panel-content-wrapper',
        autoRefresh: true
      },
      hide: {
        onClick: true,
...
module.exports = BaseRegion.extend({
  ...
  constructor: function constructor() {
    ...
    var PanelLayoutView = __webpack_require__(430);
  ...
  setSize: function setSize() {
    var width = this.storage.size.width,
        side = madxartworkCommon.config.isRTL ? 'right' : 'left';
    this.$el.css('width', width);
    madxartwork.$previewWrapper.css(side, width);
  },
...
var _component = _interopRequireDefault(__webpack_require__(431));

var _component2 = _interopRequireDefault(__webpack_require__(432));

var _component3 = _interopRequireDefault(__webpack_require__(433));

var EditModeItemView = __webpack_require__(434),
    PanelLayoutView;

PanelLayoutView = Marionette.LayoutView.extend({
  template: '#tmpl-madxartwork-panel',
  id: 'madxartwork-panel-inner',
  regions: {
    content: '#madxartwork-panel-content-wrapper',
    header: '#madxartwork-panel-header-wrapper',
    footer: '#madxartwork-panel-footer',
    modeSwitcher: '#madxartwork-mode-switcher'
  },
  pages: {},
  childEvents: {
  ...
  buildPages: function buildPages() {
    var pages = {
      elements: {
        view: __webpack_require__(435),
        title: '<img src="' + madxartworkCommon.config.urls.assets + 'images/logo-panel.svg">'
      },
      editor: {
        view: __webpack_require__(440)
      },
      menu: {
        view: madxartwork.modules.layouts.panel.pages.menu.Menu,
        title: '<img src="' + madxartworkCommon.config.urls.assets + 'images/logo-panel.svg">'
      },
      colorScheme: {
    ...
    var schemesTypes = (0, _keys.default)(madxartwork.schemes.getSchemes()),
        disabledSchemes = _.difference(schemesTypes, madxartwork.schemes.getEnabledSchemesTypes());

    _.each(disabledSchemes, function (schemeType) {
      var scheme = madxartwork.schemes.getScheme(schemeType);
      pages[schemeType + 'Scheme'].view = __webpack_require__(445).extend({
  ...
  setPage: function setPage(page, title, viewOptions) {
    ...
    if ('elements' === page && !madxartwork.userCan('design')) {
    ...
    if (!pageData) {
      throw new ReferenceError('madxartwork panel doesn\'t have page named \'' + page + '\'');
    }
  ...
  onBeforeShow: function onBeforeShow() {
    var PanelFooterItemView = __webpack_require__(446),
        PanelHeaderItemView = __webpack_require__(447); // Edit Mode
...
function (_madxartworkModules$com) {
  ...
  (0, _createClass2.default)(_default, [{
    ...
    value: function defaultCommands() {
      return {
        open: function open() {
          return madxartwork.getPanelView().modeSwitcher.currentView.setMode('edit');
        },
        close: function close() {
          return madxartwork.getPanelView().modeSwitcher.currentView.setMode('preview');
        },
        toggle: function toggle() {
          return madxartwork.getPanelView().modeSwitcher.currentView.toggleMode();
        },
        save: function save() {
          return madxartwork.saver.saveDraft();
        },
        publish: function publish() {
          return madxartwork.saver.publish();
        },
        exit: function exit() {
        ...
        'change-device-mode': function changeDeviceMode(args) {
          ...
          if (!args.device) {
            var currentDeviceMode = madxartwork.channels.deviceMode.request('currentMode');
          ...
          madxartwork.changeDeviceMode(args.device);
        }
      };
    }
  }, {
...
function (_madxartworkModules$com) {
  ...
  (0, _createClass2.default)(_default, [{
    ...
    value: function defaultTabs() {
      return {
        categories: {
          title: madxartwork.translate('elements')
        },
        global: {
          title: madxartwork.translate('global')
        }
      };
    }
  }, {
...
function (_madxartworkModules$com) {
  ...
  (0, _createClass2.default)(_default, [{
    ...
    value: function defaultTabs() {
      return {
        content: {
          title: madxartwork.translate('content')
        },
        style: {
          title: madxartwork.translate('style')
        },
        ...
        layout: {
          title: madxartwork.translate('layout')
        }
      };
    }
  }, {
    ...
    value: function defaultCommands() {
      ...
      return {
        open: function open(args) {
          ...
          madxartwork.hooks.doAction(action, _this.manager, args.model, args.view); // Example: panel/open_editor/widget/heading

          madxartwork.hooks.doAction(action + '/' + args.model.get('widgetType'), _this.manager, args.model, args.view);
        }
      };
    }
  }, {
    ...
    value: function setDefaultTab(args) {
      ...
      if (this.activeTabs[args.model.id]) {
        ...
        defaultTab = jQuery(this.getTabsWrapperSelector()).find('.madxartwork-component-tab').eq(0).data('tab');
      }
    ...
    value: function openEditor(model, view) {
      var title = madxartwork.translate('edit_element', [madxartwork.getElementData(model).title]),
          editor = madxartwork.getPanelView().setPage('editor', title, {
        model: model,
        controls: madxartwork.getElementControls(model),
...
EditModeItemView = Marionette.ItemView.extend({
  template: '#tmpl-madxartwork-mode-switcher-content',
  id: 'madxartwork-mode-switcher-inner',
  ui: {
    previewButton: '#madxartwork-mode-switcher-preview-input',
    previewLabel: '#madxartwork-mode-switcher-preview',
    previewLabelA11y: '#madxartwork-mode-switcher-preview .madxartwork-screen-only'
  },
  events: {
    'change @ui.previewButton': 'onPreviewButtonChange'
  },
  initialize: function initialize() {
    this.listenTo(madxartwork.channels.dataEditMode, 'switch', this.onEditModeChanged);
  },
  getCurrentMode: function getCurrentMode() {
  ...
  onPreviewButtonChange: function onPreviewButtonChange() {
    madxartwork.changeEditMode(this.getCurrentMode());
  },
  onEditModeChanged: function onEditModeChanged() {
    var activeMode = madxartwork.channels.dataEditMode.request('activeMode'),
        title = madxartwork.translate('preview' === activeMode ? 'back_to_editor' : 'preview');
...
var PanelElementsCategoriesCollection = __webpack_require__(436),
    PanelElementsElementsCollection = __webpack_require__(260),
    PanelElementsCategoriesView = __webpack_require__(437),
    PanelElementsElementsView = madxartwork.modules.layouts.panel.pages.elements.views.Elements,
    PanelElementsSearchView = __webpack_require__(439),
    PanelElementsGlobalView = __webpack_require__(257),
    PanelElementsLayoutView;

PanelElementsLayoutView = Marionette.LayoutView.extend({
  template: '#tmpl-madxartwork-panel-elements',
  id: 'madxartwork-panel-page-elements',
  options: {
    autoFocusSearch: true
  },
  regions: {
    elements: '#madxartwork-panel-elements-wrapper',
    search: '#madxartwork-panel-elements-search-area'
  },
  regionViews: {},
  elementsCollection: null,
  categoriesCollection: null,
  initialize: function initialize() {
    this.listenTo(madxartwork.channels.panelElements, 'element:selected', this.destroy);
  ...
  initRegionViews: function initRegionViews() {
    ...
    this.regionViews = madxartwork.hooks.applyFilters('panel/elements/regionViews', regionViews);
  },
  initElementsCollection: function initElementsCollection() {
    var elementsCollection = new PanelElementsElementsCollection(),
        sectionConfig = madxartwork.config.elements.section;
    elementsCollection.add({
      title: madxartwork.translate('inner_section'),
      elType: 'section',
      categories: ['basic'],
    ...
    _.each(madxartwork.config.widgets, function (widget) {
      if (madxartwork.config.document.panel.widgets_settings[widget.widget_type]) {
        widget = _.extend(widget, madxartwork.config.document.panel.widgets_settings[widget.widget_type]);
      }
  ...
  initCategoriesCollection: function initCategoriesCollection() {
    ...
    _.each(madxartwork.config.document.panel.elements_categories, function (categoryConfig, categoryName) {
  ...
  changeFilter: function changeFilter(filterValue) {
    madxartwork.channels.panelElements.reply('filter:value', filterValue).trigger('filter:change');
  },
  ...
  focusSearch: function focusSearch() {
    if (!madxartwork.userCan('design') || !this.search
  ...
  onDestroy: function onDestroy() {
    madxartwork.channels.panelElements.reply('filter:value', null);
  },
...
var PanelElementsCategoryView = __webpack_require__(438),
    PanelElementsCategoriesView;

PanelElementsCategoriesView = Marionette.CompositeView.extend({
  template: '#tmpl-madxartwork-panel-categories',
  childView: PanelElementsCategoryView,
  childViewContainer: '#madxartwork-panel-categories',
  id: 'madxartwork-panel-elements-categories',
  initialize: function initialize() {
    this.listenTo(madxartwork.channels.panelElements, 'filter:change', this.onPanelElementsFilterChange);
  },
  onPanelElementsFilterChange: function onPanelElementsFilterChange() {
    if (madxartwork.channels.panelElements.request('filter:value')) {
      madxartwork.getPanelView().getCurrentPageView().showView('elements');
    }
  }
});
...
PanelElementsCategoryView = Marionette.CompositeView.extend({
  template: '#tmpl-madxartwork-panel-elements-category',
  className: 'madxartwork-panel-category',
  ui: {
    title: '.madxartwork-panel-category-title',
    items: '.madxartwork-panel-category-items'
  },
  events: {
    'click @ui.title': 'onTitleClick'
  },
  id: function id() {
    return 'madxartwork-panel-category-' + this.model.get('name');
  },
  childView: __webpack_require__(258),
  childViewContainer: '.madxartwork-panel-category-items',
  initialize: function initialize() {
    this.collection = new PanelElementsElementsCollection(this.model.get('items'));
  },
  onRender: function onRender() {
    var isActive = madxartwork.channels.panelElements.request('category:' + this.model.get('name') + ':active');
    ...
    if (isActive) {
      this.$el.addClass('madxartwork-active');
      this.ui.items.show();
    }
  },
  onTitleClick: function onTitleClick() {
    var $items = this.ui.items,
        activeClass = 'madxartwork-active',
        isActive = this.$el.hasClass(activeClass),
        slideFn = isActive ? 'slideUp' : 'slideDown';
    madxartwork.channels.panelElements.reply('category:' + this.model.get('name') + ':active', !isActive);
...
PanelElementsSearchView = Marionette.ItemView.extend({
  template: '#tmpl-madxartwork-panel-element-search',
  id: 'madxartwork-panel-elements-search-wrapper',
  ui: {
    input: 'input'
  },
  events: {
...
var ControlsStack = madxartworkModules.editor.views.ControlsStack,
    EditorView;
EditorView = ControlsStack.extend({
  template: Marionette.TemplateCache.get('#tmpl-editor-content'),
  id: 'madxartwork-panel-page-editor',
  childViewContainer: '#madxartwork-controls',
  childViewOptions: function childViewOptions() {
    return {
  ...
  getNamespaceArray: function getNamespaceArray() {
    var eventNamespace = madxartworkModules.editor.views.ControlsStack.prototype.getNamespaceArray();
  ...
  openActiveSection: function openActiveSection() {
    ...
    madxartwork.channels.editor.trigger('section:activated', this.activeSection, this);
  },
  isVisibleSectionControl: function isVisibleSectionControl(sectionControlModel) {
    return ControlsStack.prototype.isVisibleSectionControl.apply(this, arguments) && madxartwork.helpers.isActiveControl(sectionControlModel, this.model.get('settings').attributes);
  },
  scrollToEditedElement: function scrollToEditedElement() {
    madxartwork.helpers.scrollToView(this.getOption('editedElementView').$el);
  },
  onDestroy: function onDestroy() {
    var editedElementView = this.getOption('editedElementView');

    if (editedElementView) {
      editedElementView.$el.removeClass('madxartwork-element-editable');
    }
  ...
  onRender: function onRender() {
    ...
    if (editedElementView) {
      editedElementView.$el.addClass('madxartwork-element-editable');
    }
  },
  ...
  onChildviewSettingsChange: function onChildviewSettingsChange(childView) {
    ...
    madxartwork.channels.editor.trigger('change', childView, editedElementView).trigger('change:' + editedElementType, childView, editedElementView).trigger('change:' + editedElementType + ':' + childView.model.get('name'), childView, editedElementView);
  }
});
...
PanelSchemeColorView = PanelSchemeItemView.extend({
  ...
  ui: {
    input: '.madxartwork-panel-scheme-color-value'
  },
...
PanelSchemeTypographyView = PanelSchemeItemView.extend({
  ...
  className: function className() {
    ...
    return classes + ' madxartwork-panel-box';
  },
  ui: {
    heading: '.madxartwork-panel-heading',
    allFields: '.madxartwork-panel-scheme-typography-item-field',
    inputFields: 'input.madxartwork-panel-scheme-typography-item-field',
    selectFields: 'select.madxartwork-panel-scheme-typography-item-field',
    selectFamilyFields: 'select.madxartwork-panel-scheme-typography-item-field[name="font_family"]'
  },
  events: {
  ...
  toggleVisibility: function toggleVisibility() {
    this.$el.toggleClass('madxartwork-open');
  },
  changeUIValue: function changeUIValue(newValue) {
  ...
  onFieldChange: function onFieldChange(event) {
    var $select = this.$(event.currentTarget),
        currentValue = madxartwork.schemes.getSchemeValue('typography', this.model.get('key')).value,
        fieldKey = $select.attr('name');
    currentValue[fieldKey] = $select.val();

    if ('font_family' === fieldKey && !_.isEmpty(currentValue[fieldKey])) {
      madxartwork.helpers.enqueueFont(currentValue[fieldKey]);
    }
...
PanelSchemeColorPickerView = PanelSchemeColorsView.extend({
  ...
  orderView: function orderView(model) {
    return madxartwork.helpers.getColorPickerPaletteIndex(model.get('key'));
  }
});
...
PanelSchemeDisabledView = Marionette.ItemView.extend({
  template: '#tmpl-madxartwork-panel-schemes-disabled',
  id: 'madxartwork-panel-schemes-disabled',
  className: 'madxartwork-nerd-box',
  disabledTitle: '',
  templateHelpers: function templateHelpers() {
    return {
...
module.exports = Marionette.ItemView.extend({
  template: '#tmpl-madxartwork-panel-footer-content',
  tagName: 'nav',
  id: 'madxartwork-panel-footer-tools',
  possibleRotateModes: ['portrait', 'landscape'],
  ui: {
    menuButtons: '.madxartwork-panel-footer-tool',
    settings: '#madxartwork-panel-footer-settings',
    deviceModeIcon: '#madxartwork-panel-footer-responsive > i',
    deviceModeButtons: '#madxartwork-panel-footer-responsive .madxartwork-panel-footer-sub-menu-item',
    saveTemplate: '#madxartwork-panel-footer-sub-menu-item-save-template',
    history: '#madxartwork-panel-footer-history',
    navigator: '#madxartwork-panel-footer-navigator'
  },
  events: {
  ...
  behaviors: function behaviors() {
    var behaviors = {
      saver: {
        behaviorClass: madxartwork.modules.components.saver.behaviors.FooterSaver
      }
    };
    return madxartwork.hooks.applyFilters('panel/footer/behaviors', behaviors, this);
  },
  initialize: function initialize() {
    this.listenTo(madxartwork.channels.deviceMode, 'change', this.onDeviceModeChange);
  },
  ...
  addSubMenuItem: function addSubMenuItem(subMenuName, itemData) {
    var $newItem = jQuery('<div>', {
      id: 'madxartwork-panel-footer-sub-menu-item-' + itemData.name,
      class: 'madxartwork-panel-footer-sub-menu-item'
    }),
        $itemIcon = jQuery('<i>', {
      class: 'madxartwork-icon ' + itemData.icon,
      'aria-hidden': true
    }),
        $itemTitle = jQuery('<div>', {
      class: 'madxartwork-title'
    }).text(itemData.title);
    $newItem.append($itemIcon, $itemTitle);

    if (itemData.description) {
      var $itemDescription = jQuery('<div>', {
        class: 'madxartwork-description'
    ...
    var $menuTool = this.ui.menuButtons.filter('#madxartwork-panel-footer-' + subMenuName);

    if (itemData.before) {
      var $beforeItem = $menuTool.find('#madxartwork-panel-footer-sub-menu-item-' + itemData.before);
    ...
    var $subMenu = $menuTool.find('.madxartwork-panel-footer-sub-menu');
  ...
  onMenuButtonsClick: function onMenuButtonsClick(event) {
    ...
    if (!$tool.hasClass('madxartwork-toggle-state') || jQuery(event.target).closest('.madxartwork-panel-footer-sub-menu-item').length) {
      return;
    }

    var isOpen = $tool.hasClass('madxartwork-open');
    this.ui.menuButtons.not('.madxartwork-leave-open').removeClass('madxartwork-open');

    if (!isOpen) {
      $tool.addClass('madxartwork-open');
    }
  },
  ...
  onDeviceModeChange: function onDeviceModeChange() {
    var previousDeviceMode = madxartwork.channels.deviceMode.request('previousMode'),
        currentDeviceMode = madxartwork.channels.deviceMode.request('currentMode');
  ...
  onResponsiveButtonsClick: function onResponsiveButtonsClick(event) {
    ...
    madxartwork.changeDeviceMode(newDeviceMode);
  },
...
PanelHeaderItemView = Marionette.ItemView.extend({
  template: '#tmpl-madxartwork-panel-header',
  id: 'madxartwork-panel-header',
  ui: {
    menuButton: '#madxartwork-panel-header-menu-button',
    menuIcon: '#madxartwork-panel-header-menu-button i',
    title: '#madxartwork-panel-header-title',
    addButton: '#madxartwork-panel-header-add-button'
  },
  events: {
...
var _independent = _interopRequireDefault(__webpack_require__(449));

var _rightClickIntroduction = _interopRequireDefault(__webpack_require__(450));

var BaseSectionsContainerView = __webpack_require__(451),
    Preview;

Preview = BaseSectionsContainerView.extend({
  template: Marionette.TemplateCache.get('#tmpl-madxartwork-preview'),
  className: 'madxartwork-inner',
  childViewContainer: '.madxartwork-section-wrap',
  behaviors: function behaviors() {
    ...
    if (!madxartwork.config.user.introduction.rightClick && !madxartwork.config.user.introduction[2]) {
  ...
  paste: function paste(atIndex) {
    ...
    madxartwork.channels.data.trigger('element:before:add', transferData.elements[0]);
    ...
    madxartwork.channels.data.trigger('element:after:add', transferData.elements[0]);
  },
  isPasteEnabled: function isPasteEnabled() {
    return madxartworkCommon.storage.get('transfer');
  },
...
function (_BaseAddSectionView) {
  ...
  (0, _createClass2.default)(AddSectionView, [{
    ...
    key: "id",
    get: function get() {
      return 'madxartwork-add-new-section';
    }
  }]);
  return AddSectionView;
}(_base.default);
...
function (_Marionette$Behavior) {
  ...
  (0, _createClass2.default)(_default, [{
    ...
    value: function ui() {
      return {
        editButton: '.madxartwork-editor-element-edit'
      };
    }
  }, {
    key: "events",
      ...
      this.getIntroduction = function () {
        if (!introduction) {
          introduction = new madxartworkModules.editor.utils.Introduction({
            ...
            dialogOptions: {
              className: 'madxartwork-right-click-introduction',
              headerMessage: madxartwork.translate('meet_right_click_header'),
              message: madxartwork.translate('meet_right_click_message'),
              iframe: madxartwork.$preview,
              position: {
                my: 'center top+5',
                at: 'center bottom',
            ...
            onDialogInitCallback: function onDialogInitCallback(dialog) {
              dialog.addButton({
                name: 'learn-more',
                text: madxartwork.translate('learn_more'),
                tag: 'div',
                callback: function callback() {
                  open(madxartwork.config.help_right_click_url, '_blank');
                }
              });
              dialog.addButton({
                name: 'ok',
                text: madxartwork.translate('got_it'),
                callback: function callback() {
                  return introduction.setViewed();
                }
              });
              dialog.getElements('ok').addClass('madxartwork-button madxartwork-button-success');
            }
          });
        }
...
BaseSectionsContainerView = BaseContainer.extend({
  ...
  behaviors: function behaviors() {
    ...
    return madxartwork.hooks.applyFilters('elements/base-section-container/behaviors', behaviors, this);
  },
  getSortableOptions: function getSortableOptions() {
    return {
      handle: '> .madxartwork-element-overlay .madxartwork-editor-element-edit',
      items: '> .madxartwork-section'
    };
  },
  getChildType: function getChildType() {
    return ['section'];
  ...
  initialize: function initialize() {
    BaseContainer.prototype.initialize.apply(this, arguments);
    this.listenTo(this.collection, 'add remove reset', this.onCollectionChanged).listenTo(madxartwork.channels.panelElements, 'element:drag:start', this.onPanelElementDragStart).listenTo(madxartwork.channels.panelElements, 'element:drag:end', this.onPanelElementDragEnd);
  },
  onCollectionChanged: function onCollectionChanged() {
    madxartwork.saver.setFlagEditorChange(true);
  },
  onPanelElementDragStart: function onPanelElementDragStart() {
    ...
    this.$el.find('.madxartwork-background-video-embed').hide();
    madxartwork.helpers.disableElementEvents(this.$el.find('iframe'));
  },
  onPanelElementDragEnd: function onPanelElementDragEnd() {
    this.$el.find('.madxartwork-background-video-embed').show();
    madxartwork.helpers.enableElementEvents(this.$el.find('iframe'));
  }
});
