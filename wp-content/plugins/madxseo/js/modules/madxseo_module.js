 ...
 * @return Mixed.
 */
function madxseo_get_field_value( field ) {
	if ( field.length == 0 ) {
		return field;
	}
 ...
 * @return Mixed.
 */
function madxseo_get_field_values( field ) {
	arr = [];
	cur = jQuery( '[name=' + field + ']' );
	if ( cur.length == 0 ) {
		return field;
 ...
 * @return Mixed.
 */
function madxseo_eval_condshow_logic( statement ) {
	var lhs, rhs;
	if ( ( typeof statement ) == 'object' ) {
		lhs = statement.lhs;
		rhs = statement.rhs;
		if ( lhs !== null && ( ( typeof lhs ) == 'object' ) ) {
			lhs = madxseo_eval_condshow_logic( lhs );
		}
		if ( rhs !== null && ( typeof rhs ) == 'object' ) {
			rhs = madxseo_eval_condshow_logic( rhs );
		}
		lhs = madxseo_get_field_value( lhs );
		rhs = madxseo_get_field_value( rhs );
		switch ( statement.op ) {
			case 'NOT':
				return ( ! lhs );
			case 'AND':
 ...
 * @return Mixed.
 */
function madxseo_do_condshow_match( index, value ) {
		...
		jQuery.each(
			value, function(subopt, setting) {
				...
				if ( ( typeof setting ) == 'object' ) {
					statement = madxseo_eval_condshow_logic( setting );
					if ( ! statement ) {
						matches = false;
					}
				} else {
					if ( subopt.match( /\\\[\\\]/ ) ) { // special case for these -- pdb
						cur = madxseo_get_field_values( subopt );
						if ( jQuery.inArray( setting, cur, 0 ) < 0 ) {
							matches = false;
						}
					} else {
						cur = madxseo_get_field_value( subopt );
						if ( cur != setting ) {
							matches = false;
						}
					}
 ...
 * @param $value.
 */
function madxseo_add_condshow_handlers( index, value ) {
	if ( typeof value != 'undefined' ) {
		jQuery.each(
			value, function(subopt, setting) {
				jQuery( '[name=' + subopt + ']' ).bind(
					"change keyup", function() {
						madxseo_do_condshow_match( index, value );
					}
				);
			}
		);
...
jQuery( document ).ready(
	function() {
		if ( typeof aiosp_data != 'undefined' ) {
			if ( typeof aiosp_data.condshow != 'undefined' ) {
				madxseo_do_condshow( aiosp_data.condshow );
			}
		}
	 ...
	 * @since 2.3.16
	 */
		jQuery( '.madxseo_upload_image_label' ).on(
			'change', function() {
				this.checker = jQuery( this ).parent().find( '.madxseo_upload_image_checker' );
         ...
         * @since 2.3.13 Fixed issue #[740](https://github.com/madxartwork/madxartwork-seo-pack/issues/740)
         *
         */
        jQuery(document).ready(
            function ($) {
                jQuery('.madxseo_upload_image_button').each(
                    function () {
                        jQuery(this).madxseoImageUploader(
                            {
        ...
        jQuery(document).ready(
            function () {
                jQuery("#poststuff .madxseo_radio_type input[type='radio']").on(
                    'click', function () {
			...
			jQuery.each(
				aiosp_data.pointers, function( index, value ) {
					if ( value != 'undefined' && value.pointer_text != '' ) {
						madxseo_show_pointer( index, value );
					}
				}
			);
		}
	 ...
	 * @return boolean.
	 */
		jQuery( ".madxartwork-seo_page_madxartwork-seo-pack-modules-madxseo_feature_manager #aiosp_settings_form .madxseo_settings_left" )
		.delegate(
	 ...
	 * @return boolean.
	 */
		jQuery( ".madxartwork-seo_page_madxartwork-seo-pack-modules-madxseo_feature_manager #aiosp_settings_form" )
		.delegate(
			"input[name='Submit']", "click", function( e ) {
				e.preventDefault();
				madxseo_handle_post_url(
					'madxseo_ajax_save_settings',
					'ajax_settings_message',
					jQuery( 'form#aiosp_settings_form' ).serialize(),
					function() {
						jQuery( '.wp-has-current-submenu' ).fadeIn(
							'fast', function() {
								madxseo_handle_ajax_call(
									'madxseo_ajax_get_menu_links',
									'ajax_settings_message',
	 ...
	 * @return boolean.
	 */
		jQuery( ".madxartwork-seo_page_madxartwork-seo-pack-pro-modules-madxseo_feature_manager #aiosp_settings_form .madxseo_settings_left" )
		.delegate(
			"input[name='Submit']", "click", function( e ) {
				e.preventDefault();
				return false;
	 ...
	 * @return boolean.
	 */
		jQuery( ".madxartwork-seo_page_madxartwork-seo-pack-pro-modules-madxseo_feature_manager #aiosp_settings_form" )
		.delegate(
			"input[name='Submit']", "click", function( e ) {
				e.preventDefault();
				madxseo_handle_post_url(
		...
		var selectors =
		"div.madxseo_multicheckbox_type div.madxseo_option_div, #aiosp_sitemap_debug div.madxseo_option_div, #aiosp_performance_status div.madxseo_option_div";
	 ...
	 * @since 1.0.0
	 */
		jQuery( ".madxseo_tab:not(:first)" ).hide();

		/**
	 * @since 1.0.0
	 */
		jQuery( ".madxseo_tab:first" ).show();

		/**
	 * @since 1.0.0
	 * @return boolean.
	 */
		jQuery( "a.madxseo_header_tab" ).click(
			function() {
				var stringref = jQuery( this ).attr( "href" ).split( '#' )[1];
				jQuery( '.madxseo_tab:not(#' + stringref + ')' ).hide( 'slow' );
				jQuery( '.madxseo_tab#' + stringref ).show( 'slow' );
				jQuery( '.madxseo_header_tab[href!="#' + stringref + '"]' ).removeClass( 'active' );
				jQuery( '.madxseo_header_tab[href="#' + stringref + '"]' ).addClass( 'active' );
				return false;
			}
		);
    ...
    jQuery( "div#aiosp_robots_default_metabox" )
		.delegate(
			"a.aiosp_robots_edit_rule", "click", function( e ) {
                ...
                jQuery('input.add-edit-rule').val(jQuery('.madxseo_table').attr('data-edit-label'));
 ...
 */
jQuery.fn.madxseoImageUploader = function( options ) {
 ...
 * @param $success.
 */
function madxseo_handle_ajax_call( action, settings, options, success ) {
	var madxseo_sack = new sack( ajaxurl );
	madxseo_sack.execute = 1;
	madxseo_sack.method = 'POST';
	madxseo_sack.setVar( "action", action );
	madxseo_sack.setVar( "settings", settings );
	madxseo_sack.setVar( "options", options );
	if ( typeof success != 'undefined' ) {
		madxseo_sack.onCompletion = success;
	}
	madxseo_sack.setVar(
		"nonce-madxseo",
		jQuery( 'input[name="nonce-madxseo"]' ).val()
	);
	madxseo_sack.setVar(
		"nonce-madxseo-edit",
		jQuery( 'input[name="nonce-madxseo-edit"]' ).val()
	);
	madxseo_sack.onError = function() {
 ...
 * @param $success.
 */
function madxseo_handle_post_url( action, settings, options, success_function, use_native) {
	jQuery( "div#aiosp_" + settings ).fadeOut(
		'fast', function() {
			var loading = '<label class="madxseo_loading madxseo_' + settings + '_loading"></label> Please wait...';
                    ...
                    if(use_native) {
                        jQuery.ajax({
                            ...
                            data    : {
                                ...
                                'options'   : options,
                                'settings'  : settings,
                                'nonce-madxseo': jQuery( 'input[name="nonce-madxseo"]' ).val(),
                                'nonce-madxseo-edit': jQuery( 'input[name="nonce-madxseo-edit"]' ).val()
                            },
					    ...
					    madxseo_handle_ajax_call( action, settings, options, success_function );
                    }
				}
			);
...
/**
 * @summary Handles when madxseo is overflowed.
 *
 * @since 1.0.0
 * @param $element.
 * @return mixed.
 */
function madxseo_is_overflowed( element ) {
 ...
 * @param $el.
 */
function madxseo_overflow_border( el ) {
	if ( madxseo_is_overflowed( el ) ) {
		el.className = 'madxseo_option_div madxseo_overflowed';
	} else {
		el.className = 'madxseo_option_div';
	}
}
...
function aiospinitSocialMetaInPosts($) {
    ...
    $('input[name="madxseo_opengraph_settings_customimg_checker"] ~ .madxseo_upload_image_button').on('click', function(e){
        $('input[name="madxseo_opengraph_settings_image"]').attr('checked', false);
    });
}
